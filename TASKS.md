...
Status:
In progress — Day61 lesson + real runtime artifacts (day61_provider_artifact_logic.py, day61_fake_provider.py, day61_provider_adapter.py, day61_artifact_store.py, day61_worker_completion.py), OTel Collector + docker-compose config, requirements-day61.txt, and the design/runbook are delivered; `py_compile` + `test_day61_provider_artifact_logic.py` + `test_day61_fake_provider_http.py` + `test_day61_lease_fencing_and_telemetry.py` = 19 passed (EXECUTED_LOCAL_RUNTIME, incl. a REAL HTTP loopback round-trip). The full local INTEGRATION_RUNTIME matrix (PostgreSQL + Redis/Celery + MinIO + OTel Collector) is NOT RUN (no Docker), so Day61 is NOT marked Completed and stays the current lesson. No new migration (schema already supports it; Day60 head 0012 + lease triple/recovery preserved); no secrets committed. REVIEW FIXES: Result-bytes metadata COMPUTED from actual bytes (success->VERIFIED); all external ops + state changes lease-token fenced (stale Worker: no Provider call, no successor state change); provider_request_id immutable (NULL->set/same->idempotent/diff->conflict); minimal real OTel (day61_telemetry.py). REVIEW ROUND 2: fake Provider idempotent on the stable correlation key (one external operation/key; same-key retry reuses provider_request_id + result; incompatible mode -> HTTP 409); operation_span() swallows only telemetry errors, never a business exception (no double-yield); configurable disabled-by-default OTLP exporter (init_telemetry) + W3C traceparent inject/extract carried on the existing Outbox payload JSONB (no migration); pre-call Attempt-ownership check -> attempt_mismatch_no_external_call before any Provider HTTP. REVIEW ROUND 3: W3C trace context wired acceptance->Outbox payload->Relay->Celery kwargs->Worker extract->Provider/Storage/DB spans (retry reuses same trace association); init_telemetry() bootstrapped at FastAPI lifespan / Relay main / Celery worker_process_init (idempotent, disabled by default); stale Worker timeout/invalid-response returns lease_lost_no_commit (no fabricated transition, successor Job untouched). REVIEW ROUND 4: the production Celery task runs day61_worker_runtime.run_authoritative_attempt (guarded claim -> tenant/correlation from durable PG facts -> run_external_operation under the lease token; outcome verbatim) so a Job succeeds ONLY after a real Provider+Artifact — no 'no-Provider straight-to-succeeded' path (Day60 run_worker_attempt is teaching-only); acceptance opens a fastapi.accept_job ROOT span so the traceparent is really emitted into the Outbox payload. Tests 9 -> 19 -> 30 -> 45 -> 61 -> 64 passed. INTEGRATION_RUNTIME still NOT RUN; Day61 NOT Completed.

Target lesson:
Not created yet — see CURRICULUM.md and ROADMAP.md.

(Semantics, shared with PROJECT_STATUS.md: "Current Lesson" = the lesson currently being worked on / next up (Day61);
Day60 is finished and recorded as PROJECT_STATUS.md's "Last Completed Lesson". Phase 5 Production Integration Gate is in progress.)
