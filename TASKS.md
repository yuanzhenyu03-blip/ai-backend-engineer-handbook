...
Phase 7A is COMPLETE at classroom scope. Phase 7B is IN PROGRESS: Day86 is COMPLETE at guided classroom scope and Day87 is next.

Phase 6 — n8n AI Workflow Integration is COMPLETE (Day67–Day70 Completed). Phase 7A (LLM Application Engineering) begins at Day71 as a phase transition (foundations Day53-Day61, not an n8n dependency). Phase 5 — Production Runtime Integration and Browser Tool Engineering is COMPLETE (previous phase): Day60 and Day61 (the Production Integration Gate) are COMPLETE; Day61 has disposable local INTEGRATION_RUNTIME evidence for the success chain and timeout-after-receipt recovery. Day62 (Playwright Runtime, Locators and Reliable Async Interaction) is COMPLETE as EXECUTED_LOCAL_RUNTIME (pure interaction/cleanup logic + a controlled research page over real HTTP loopback + a classroom Chromium run; the real-Chromium test suite is gated on the `playwright` package). Day63 (Browser Authentication, Storage State and Tenant Isolation) is COMPLETE (lesson + a pure Session-Gate authorization/claim decision core; EXECUTED_LOCAL_RUNTIME; the LIVE classroom artifact was CONCEPTUAL_STATIC; real Chromium isolation, real PostgreSQL atomic claim and a credential/secret store are NOT RUN). Day64 (Dynamic Extraction, Network Events and Artifact Evidence) is COMPLETE (lesson + a pure Extraction/Artifact decision core; EXECUTED_LOCAL_RUNTIME; the LIVE classroom artifact was CONCEPTUAL_STATIC; real Playwright extraction, real Object Storage HEAD and a real PostgreSQL Artifact-reference transaction are NOT RUN). Day65 (Browser Failure Recovery and Security Boundaries) is COMPLETE (lesson + a pure recovery/security decision core; EXECUTED_LOCAL_RUNTIME; the LIVE classroom artifact was CONCEPTUAL_STATIC; real Playwright/traces/SSRF/CAPTCHA/audit-lookup/Worker/queue are NOT RUN). Day66 (Queue-backed Playwright Worker as a Permissioned AI Tool) is COMPLETE (lesson + a pure queue-backed permissioned-worker decision core reusing the Day63 fence + Day65 recovery; EXECUTED_LOCAL_RUNTIME; the LIVE classroom artifact was CONCEPTUAL_STATIC; a real Provider/LLM loop, guarded PostgreSQL claim, Outbox Relay/Broker, Celery ACK/redelivery, lease recovery, Playwright, Session cancellation, and Object Storage are NOT RUN), completing Phase 5. Day67 (n8n Workflow Model, Triggers, FastAPI Integration and Responsibility Boundaries) is COMPLETE (Phase 6 begins): a lesson + a `projects/n8n-workflows/` draft-workflow configuration record; n8n is permissioned orchestration only and FastAPI keeps the Day66 ownership boundaries; one classroom EXECUTED_LOCAL_RUNTIME 400 proof; a valid FastAPI path, service auth, durable Task, PostgreSQL, queue/Outbox, worker execution, and a published Production URL are NOT RUN / NOT CONFIGURED, with no exported workflow JSON. Day68 (Long-running AI Jobs: Polling, Callback, Correlation and Idempotency) is COMPLETE (classroom scope): a lesson + the Day68 Long-running Job Orchestration Contract in `projects/n8n-workflows/`; CONCEPTUAL_STATIC only — the n8n workflow runtime, a valid FastAPI acceptance/status integration, real polling/callback/PostgreSQL/Worker/Provider behaviours, and production are NOT RUN, with no exported workflow JSON. Day69 (Human Approval, Retry, Secrets, Audit and Error Workflows) is COMPLETE (classroom scope): a lesson + the Day69 Risk-based Approval Gate contract in `projects/n8n-workflows/`; risk-based approval + classified recovery + Secret boundaries + authoritative audit + incident hardening; CONCEPTUAL_STATIC only — the n8n workflow runtime, a valid FastAPI approval/publication integration, real approval/callback/PostgreSQL/credential-store/Provider behaviours, and production are NOT RUN, with no exported workflow JSON. Day70 (n8n + FastAPI + AI Tool Integration Capstone and Interview) is COMPLETE, closing Phase 6: a capstone under `projects/n8n-workflows/` (DAY70_CAPSTONE.md + day70_capstone.py + test_day70_capstone.py (21 tests; the classroom's 14 pre-fix tests superseded) + a Secret-free importable day70_minimal_acceptance_workflow.json with two independent auth boundaries and a `{document_ids, business_input:{report_scope}}` body). A bounded real, authenticated n8n -> FastAPI/Uvicorn -> PostgreSQL SERVICE-call ACCEPTANCE slice reached INTEGRATION_RUNTIME (in class) for the PRE-FIX workflow — it did NOT prove inbound caller->n8n auth and is NOT evidence for the FIXED workflow (NOT RERUN); the decision model + workflow static contract are EXECUTED_LOCAL_RUNTIME (21 passed; affected Day59 fingerprint test 12 passed); inbound Webhook authentication is NOT RUN / NOT CONFIGURED; the rest is CONCEPTUAL_STATIC / NOT RUN. Day71 (LLM Application Architecture, Tokens, Context, Sampling and Model Failure Modes, Phase 7A) is COMPLETE (classroom scope, PHASE_TRANSITION built on Day53-Day61, not an n8n dependency; CONCEPTUAL + STATIC only, runtime NOT RUN); Day72 (Provider Capabilities and the Replaceable Provider Adapter, Phase 7A) is COMPLETE (classroom scope; DIRECT PREREQUISITES Day71 + Day53 + Day61; CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME 58 in-process tests; INTEGRATION/PRODUCTION NOT RUN); Day73 (Prompt Contracts, Prompt Versioning and Compatibility, Phase 7A) is COMPLETE (classroom scope; DIRECT PREREQUISITES Day72 + Day71 + Day53 + Day54; the application-owned Prompt Contract + immutable per-Attempt binding + fail-closed pre-Provider LLM Runtime gate before Day72 capability admission; CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME 39 Day73 in-process tests / 97 total; INTEGRATION/PRODUCTION NOT RUN); Day74 (Structured Output, JSON Schema and Function/Tool Calling) is COMPLETE (classroom scope; 34 Day74 tests / 131 total); Day75 (Streaming, Caching and Batching for LLM Applications) is COMPLETE (classroom scope; 41 Day75 tests / 172 total); Day76 (Model Routing, Fallback, Latency and Cost Engineering) is COMPLETE (classroom scope; 18 Day76 tests / 190 total); Day77 (Fake Provider, Contract Tests and LLM Regression Tests) is COMPLETE (31 Day77 tests / 221 total); Day78 is COMPLETE, closing Phase 7A at classroom scope; Day79–Day86 are COMPLETE at recorded classroom scope; Phase 7B is IN PROGRESS and Day87 is next. Phase 4 (Day43-Day58) remains COMPLETE as classroom scope + deterministic EXECUTED_LOCAL_RUNTIME artifacts.

---

## Current Lesson

Day87 — Agent Framework and Job-Market Refresh Checkpoint (Phase 7B) — next up.
Most recent classroom lesson: Day86 — Agent Security: Prompt Injection, Tool Abuse, Data Exfiltration and Sandboxing (included in this repository revision).

Day86 completion record:

- [x] Release the 16-section lesson with first-use security vocabulary and application-owned trust boundaries.
- [x] Extend `projects/ai-agent/` with content provenance, current Tool/Egress/Sandbox admission, exact semantic
  checks, Fake effect ports, result verification, reconciliation, security fan-in and incident containment.
- [x] Add 28 behavior tests, 3 grader tests, 25 version-1 seed cases, a deterministic example, design,
  actual classroom record and validation evidence.
- [x] Run 474 cumulative tests and Day83 26/26, Day84 16/16, Day85 18/18 and Day86 25/25 seed suites on Python 3.11.5.
- [x] Preserve teacher/student attribution: guided classroom complete; instructor-authored final synthesis;
  independent final synthesis NOT ASSESSED; only AS16 explicitly independently reviewed by the learner.
- [ ] Independently reassess the Day86 final synthesis and remaining seed expectations before a release gate.
- [ ] Verify Python 3.12 and real Provider/Tool/Egress/OS-Sandbox/Secret-manager/PostgreSQL/Broker/Worker integration when authorized.

Day85 completion record:

- [x] Release the 16-section lesson with first-use vocabulary and explicit Supervisor/Coordinator/Worker boundaries.
- [x] Extend the continuing `projects/ai-agent/` Artifact with handoff acceptance, bounded grants and allocations,
  Outbox recovery, claim/lease/fence, dispatch, reconciliation, cancellation, result verification and fan-in.
- [x] Add 29 behavior tests, 3 grader tests, 18 version-1 seed cases, a deterministic example, design,
  actual classroom record, validation evidence and repository update report.
- [x] Run 443 cumulative tests, 26/26 Day83, 16/16 Day84 and 18/18 Day85 seed suites on Python 3.11.5.
- [x] Preserve teacher/student attribution: guided classroom complete; instructor-authored final synthesis;
  independent final synthesis NOT ASSESSED.
- [ ] Independently reassess the Day85 final synthesis and seed expectations before a release gate.
- [ ] Verify Python 3.12 and real PostgreSQL/Outbox Relay/Broker/Worker/Provider integration when authorized.

Day84 completion record:

- [x] Release the 16-section lesson under `docs/fastapi/` with context assembly, bounded validated Tool views,
  compaction and current-authority rehydration kept under the repository-defined title.
- [x] Extend the continuing `projects/ai-agent/` Artifact with `context_memory.py`, 33 focused tests,
  16 version-1 seed cases, a deterministic example, design, classroom record and validation evidence.
- [x] Run 411 cumulative tests, the 26/26 Day83 seed regression and the 16/16 Day84 seed suite on Python 3.11.5.
- [x] Preserve teacher/student attribution: guided classroom complete; instructor-authored final synthesis;
  independent final synthesis NOT ASSESSED.
- [ ] Independently reassess the Day84 final synthesis and seed expectations before a release gate.
- [ ] Verify Python 3.12 and real tokenizer/summarizer/Provider/database/storage/queue integration when authorized.

Status:
Released — Day71 LLM Application Architecture, Tokens, Context, Sampling and Model Failure Modes under `projects/ai-agent/` (Phase 7A begins): the lesson `docs/fastapi/day71-llm-application-architecture-tokens-context-sampling-and-model-failure-modes.md` plus the Phase 7A foundations `projects/ai-agent/docs/DAY71_FOUNDATIONS.md` (released static provider-independent design contract) and `projects/ai-agent/docs/day71-llm-runtime-foundations-classroom-draft.md`, with `cheat_sheets/fastapi.md` + `interview/fastapi.md` Day71 appends and the ai-agent README progress updated. Day70 -> Day71 is a PHASE_TRANSITION (Phase 6 -> Phase 7A); technical prerequisites are Day53 (Provider boundary/structured output) -> Day54 (streaming/lifecycle) -> Day56 (resilience/token cost/backpressure) -> Day57 (fake Provider/contract tests) -> Day58 (observability/evidence) -> Day61 (Provider Adapter HTTP evidence); n8n has NO direct dependency on Day71. Core model: the application owns a Provider-independent contract; the Provider is a versioned capability behind an Adapter that must never silently weaken the contract; a token is a model-specific unit and the context window is a finite shared budget (input + reserved output + margin <= limit; reserved output is capacity, not a promise); overflow policy (TRUNCATE/REJECT/SUMMARIZE-lossy/CHUNK) is a product decision (evidence-critical -> chunk; aggregate bounded candidates with provenance; RESOLVED/CONFLICT/INSUFFICIENT_EVIDENCE); sampling (temperature/top_p) is selection behaviour, not factual control (deterministic != reproducible != true); validation is layered (transport -> schema -> evidence -> policy -> guarded completion) and HTTP 200 + valid schema do not prove correctness; failure taxonomy = hallucination/unsupported, contradicted, format, truncation, timeout-unknown, provider-failure (root cause vs symptom: output-budget exhaustion = primary TRUNCATION, incomplete JSON = derived FORMAT_FAILURE); recovery = retry/reject/reconciliation (timeout after dispatch = TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION, hold Attempt identity + cost uncertainty, no blind retry; late result passes the same gates, schema-valid-but-unsupported = COMPLETED_INVALID); Job (stable) vs Attempt (one execution; new call = new Attempt, never overwritten/deleted); guarded completion requires a live Job + the exact authorized Attempt (stale/late = zero-effect); rollback stops future harm and never erases calls/cost/outputs/history. Evidence: CONCEPTUAL (completed) + STATIC (PASS: sections/fences/whitespace/credential scans). NOT RUN: EXECUTED_LOCAL_RUNTIME (no runtime code or tests), INTEGRATION_RUNTIME (no Provider/DB/external process), PRODUCTION. Day70 is COMPLETE. No real or paid Provider call; no secrets, tokens, real URLs, tenant data, or Provider payloads committed. Provider token counts/context sizes/sampling behaviour are versioned capabilities, not permanent facts; the final Chinese Mental Model synthesis was supplied by the Tech Lead at the student's explicit request. Day53-Day61 runtime evidence is a named prerequisite, not Day71 validation.

Released — Day70 n8n + FastAPI + AI Tool Integration Capstone and Interview under `projects/n8n-workflows/`: the lesson `docs/fastapi/day70-n8n-fastapi-ai-tool-integration-capstone-and-interview.md` plus the capstone artifacts `DAY70_CAPSTONE.md`, `day70_capstone.py` (standalone decision model: acceptance recovery, event dedupe/conflict, exact Approval binding, Publication recovery, credential classification, incident Task classification, polling boundary), `test_day70_capstone.py` (28 tests; the classroom's 14 pre-fix tests superseded), and `day70_minimal_acceptance_workflow.json` (Secret-free importable Workflow SOURCE, two independent auth boundaries — inbound Webhook `headerAuth` and outbound HTTP `httpHeaderAuth`, both Credential Store references only, `{document_ids, business_input:{report_scope}}` body — NOT a captured post-run export). Cumulative path: authenticated-service trigger (inbound caller->n8n auth NOT RUN) -> map/validate report_scope+request_id+document_id -> FastAPI durable acceptance -> honest 202 + stable Task id -> observe same Task -> permissioned execution -> verified protected Artifact -> exact-version Approval -> idempotent Publication -> correlated audit. Boundary unchanged (n8n orchestrates; FastAPI/PostgreSQL owns durable truth/authz/idempotency/transitions/recovery/audit). Evidence (four-tier taxonomy): EXECUTED_LOCAL_RUNTIME — `day70_capstone.py` + the workflow static contract via `test_day70_capstone.py` (classroom pre-fix suite Python 3.11.5 = 14 passed, SUPERSEDED; a first attempt on Python 3.9.6 reported pytest missing = NOT RUN, not a failure; the updating agent re-ran the FIXED suite on Python 3.10.12 = 21 passed and the affected Day59 fingerprint test = 12 passed; repository-standard Python 3.12 = NOT RUN) + a real n8n inspection alone (n8n 2.25.6, /healthz 200). INTEGRATION_RUNTIME (performed in class against disposable local infra; NOT re-run by the updating agent) for the PRE-FIX workflow (top-level report_scope; no inbound Webhook auth) — a bounded real, authenticated n8n Workflow -> FastAPI/Uvicorn -> PostgreSQL SERVICE-call ACCEPTANCE slice: POST /webhook/day70/research-report -> 202 + stable Task id; exact redelivery -> same id + idempotency_replayed=true; new-connection DB jobs=1/outbox=1/document_links=1 + 64-char fingerprint + queued; invalid input -> 400 with zero Jobs. This proves the acceptance boundary ONLY; it did NOT prove inbound caller->n8n auth and is NOT evidence for the FIXED workflow (NOT RERUN; the different-report_scope 409 is proven only by the Day59 fingerprint contract test). Day69 is COMPLETE. NOT RUN: budget reservation in the acceptance transaction; a FastAPI-persisted/returned correlation_id + propagation; real Polling/Callback/Approval/Publication/Error-Workflow runtime; real Worker/Outbox-Relay/broker/Browser-Tool/Provider execution; verified Artifact generation/lookup; real Approval schema/migration/UI/callback/tenant-role authorization + atomic audit; real Publication target/operation idempotency/external reconciliation; real credential revoke/rotation/exposure review (only controlled local Credential Store injection ran); rollback/kill-switch/canary incident exercise; Python 3.12; a captured n8n post-run export; production. No real or paid Provider call; the final Chinese synthesis was taught directly by the Tech Lead after the student requested it. A green test suite or a single integration run is not cumulative integration and is not production; Day59–Day69 evidence is a named prerequisite, not Day70 validation. No secrets, tokens, real URLs, tenant data, or Provider payloads committed. Day70 closes Phase 6; Day71 is a phase transition (Day53-Day61 foundations), not an n8n dependency.

Released — Day69 Human Approval, Retry, Secrets, Audit and Error Workflows under `projects/n8n-workflows/`: the lesson `docs/fastapi/day69-human-approval-retry-secrets-audit-and-error-workflows.md` plus the Day69 Risk-based Approval Gate contract added to `projects/n8n-workflows/README.md` (Approval Gate + exact binding + lifecycle + classified retry/error matrix + Secret boundary + current-state/append-only audit + smallest-safe-boundary Error Workflow + incident runbook). Core model (Day67/Day68 boundary unchanged — n8n orchestrates; FastAPI/PostgreSQL owns durable truth/authz/idempotency/recovery/audit): human approval is RISK-BASED (an authorized tenant role, not platform staff); validation is evidence, not permission; an Approval binds exact tenant + actor + action + artifact/version + policy + expiry, so a v7 approval cannot authorize v8 (task_id/correlation_id/tenant_id stay stable; approval_id/event_id/publication operation_id/idempotency key are new); the Approval lifecycle (PENDING/APPROVED/REJECTED/EXPIRED/CANCELLED) is independent of the n8n execution lifecycle (an n8n timeout changes no state; PENDING holds until the backend-owned expires_at); retry is classified recovery not replay (HTTP timeout = OUTCOME_UNKNOWN; keep operation_id + idempotency key; query authenticated FastAPI; classify 429/503 | 400/422 | 401 stop+classify (rotate only when expiry/compromise/revocation/invalid established, else fix config) | 403 | 409 | business-terminal | unknown -> PENDING_RECONCILIATION; NOT_FOUND is ambiguous and verified before any same-key reissue — same idempotency key is not unconditional proof of safety); Secrets live only in a Credential Store (workflow holds a reference; logs/audit/export never carry a Token/Authorization header; a 401 is stopped and classified, not a blind rotate/retry); Business Audit is current-state + append-only approval_events committed atomically (not automatically tamper-proof); delivery is at-least-once (same event_id + same fingerprint = duplicate-safe no-op; same event_id + different fingerprint = integration/security conflict); the Error Workflow resumes the smallest safe operation boundary (workflow retry != business operation retry); the incident flow is contain -> revoke/rotate -> preserve -> scope -> classify -> cancel/reconcile/compensate -> verify -> regression -> controlled rollout, never deleting durable facts or reversing external effects. Evidence: CONCEPTUAL_STATIC (design review); the final Chinese synthesis was taught directly by the Tech Lead after the student requested it. Day68 is COMPLETE. NOT RUN: the Day69 n8n workflow runtime; a valid authenticated FastAPI acceptance/approval/publication integration; a real Approval UI/callback + approver auth/tenant checks; a real PostgreSQL Approval schema/migration/constraints/transactions/audit-events/Outbox; real retry/backoff/error workflow; real callback duplicate/ACK-loss/fingerprint conflict; real credential-store/revoke/rotate/redaction/access-review; real publication/notification target or external reconciliation; real Worker/Provider/Browser-Tool execution; real rollback/kill-switch/canary rollout; production. No exported workflow JSON. Day67's invalid-input local 400 and Day68's conceptual contract are NOT reused as Day69 validation; delivery is at-least-once (not exactly-once); correlation_id is not authentication; append-only audit is not automatically tamper-proof; approval is risk-based. No secrets, tokens, real callback URLs, tenant data, raw prompts, or Provider responses committed.

Released — Day68 Long-running AI Jobs: Polling, Callback, Correlation and Idempotency under `projects/n8n-workflows/`: the lesson `docs/fastapi/day68-long-running-ai-jobs-polling-callback-correlation-and-idempotency.md` plus the Day68 Long-running Job Orchestration Contract added to `projects/n8n-workflows/README.md` (Acceptance/Polling/Callback/Incident branches + the six-identity table). Core model (Day67 boundary unchanged — n8n orchestrates/observes, FastAPI/PostgreSQL owns durable truth/authz/idempotency/recovery/audit): after `202 + task_id`, an n8n execution timeout, Poll timeout, HTTP 503, or missing/duplicate/out-of-order Callback does NOT by itself change the Task's business state; acceptance retries reuse the same business `request_id` + fingerprint (same intent -> existing task_id; different meaning -> 409); Polling always observes the same `task_id` with bounded backoff/observation-deadline/max-attempts and stops only on an authoritative terminal state or reconciliation; identities are `request_id`/`task_id`/`correlation_id`/`event_id`/`task_version`/`trace_id`; Callback delivery is AT-LEAST-ONCE (authenticate -> validate -> correlate -> compute/validate event fingerprint -> atomically enforce `event_id` + fingerprint (same event_id+same meaning=idempotent no-op; same event_id+different meaning=integration/security conflict) -> `task_version` (MODELED / NOT IMPLEMENTED) ordering -> legal transition -> optional authoritative confirm -> one idempotent downstream action), correlation_id an association key (NOT authentication), and `arrival order != business-state order`; the incident flow is contain -> scope -> classify -> cancel/reconcile/compensate -> verify -> controlled rollout (never deleting durable Task facts, never blindly recreating expensive/side-effecting work). Evidence: CONCEPTUAL_STATIC (state-machine/contract design reviewed in class); the final Chinese synthesis was taught directly by the Tech Lead after the student requested it. Day67 is COMPLETE. NOT RUN: the Day68 n8n workflow runtime; a valid FastAPI acceptance/status integration; a real Polling loop (Wait/Switch, 503/backoff, deadline); real Callback reachability/authentication/duplicate/ack-loss/replay/correlation-mismatch/out-of-order behaviour; real PostgreSQL idempotency/version/terminal enforcement; real Worker/Provider duplicate-call prevention and cancellation/reconciliation; production. No exported workflow JSON. Day67's invalid-input local 400 is NOT reused as Day68 validation; delivery is at-least-once (not exactly-once). No secrets, tokens, real callback URLs, tenant data, or Provider payloads committed.

Released — Day67 n8n Workflow Model, Triggers, FastAPI Integration and Responsibility Boundaries under `projects/n8n-workflows/`: the lesson `docs/fastapi/day67-n8n-workflow-model-triggers-fastapi-integration-and-responsibility-boundaries.md` plus a draft-workflow CONFIGURATION RECORD in `projects/n8n-workflows/README.md` (`Day67 - FastAPI Orchestration Boundary`: `Webhook -> Edit Fields -> IF -> HTTP Request | Respond 400`, test path `day67/research-report`, inputs `report_scope`/`request_id`). Core model: n8n is permissioned ORCHESTRATION only; FastAPI stays the trusted business + security boundary (Day66 ownership UNCHANGED) — a workflow node may ASK FastAPI over an authenticated call but never creates/mutates a durable Task and never gets direct DB/queue/worker access; a Webhook receipt != durable acceptance (`202 + task_id` only after FastAPI commits); a shared service credential != tenant/user identity; retries are three layers (n8n transport re-send | FastAPI business idempotency collapsing the reused `request_id` | worker execution); reconciliation idempotency = `task_id + recovery_action + recovery_generation` via an authenticated FastAPI API; rollback stops orchestration first, then compensates durable facts via FastAPI, never deleting Task records or granting n8n direct DB access. Evidence: DOCUMENTATION + a single classroom EXECUTED_LOCAL_RUNTIME proof — a local invalid-webhook test returned HTTP 400 with the documented JSON error, the IF false branch + its Respond to Webhook node ran, and the HTTP Request node did NOT run (after correcting `$json...` to explicit `{{ $json... }}`; the earlier attempt misrouted and returned an empty 200). Day66 is COMPLETE. NOT RUN / NOT CONFIGURED: a valid FastAPI success path; service authentication; durable Task creation; PostgreSQL persistence; queue/Outbox dispatch; browser-worker execution; a published Production URL; production. The HTTP endpoint was an unverified local placeholder (`http://host.docker.internal:8000/api/v1/browser-tasks`) with authentication `None`; no exported workflow JSON was captured. Day66's `14 passed` and prior evidence are NOT reused as Day67 validation. No secrets, real credentials, tokens, or production endpoints committed.

Released — Day66 Queue-backed permissioned-worker decision/orchestration core under `projects/fastapi-playwright/`: `src/day66_queue_backed_permissioned_worker.py` (tool-call proposal validation with idempotency bound to a request fingerprint — same key + different fingerprint rejected, user approval necessary but not sufficient; the acceptance lifecycle boundary — Provider response = proposal, durable acceptance only at the committed transaction; atomic Browser Task + Permissioned Tool Contract + Outbox dispatch intent -> 202 + task_id, dispatched by an independent Outbox Relay AFTER commit; a minimal versioned Queue Envelope carrying identity only and never Cookies/storage state/Authorization/Provider keys, with an unsupported version dead-lettered + ACKed without loading a Job; a guarded PostgreSQL claim + lease for execution ownership; stale-write rejection via the Day63 `final_fence`; commit-before-ACK terminal dedupe; Day65 UNKNOWN_OUTCOME reconciliation hand-off; a fenced bounded-retry gate reusing `day65_recovery_security_policy.authorize_retry` where a retry is a NEW auditable Attempt; durable cooperative cancellation with fence checkpoints; a safe Tool Result boundary — verified safe summary + protected Artifact reference only; task/attempt/event/lease/trace identity + safe audit; and stale-Worker fence-removal incident classification), reusing the Day63 fence + Day65 recovery core, plus tests and the design/runbook. `python3 -m pytest -q tests/test_day66_queue_backed_permissioned_worker.py` = 14 passed, EXECUTED_LOCAL_RUNTIME. The LIVE classroom artifact was CONCEPTUAL_STATIC. Core model: the LLM proposes; the backend owns authorization, durable truth, dispatch, execution authority, recovery, and audit; `accepted != running != succeeded != published Artifact`. Day65 is COMPLETE. NOT RUN by the updating agent: a real Provider/LLM tool loop; real guarded PostgreSQL concurrent claims; a real Outbox Relay/Broker duplicate delivery; real Celery ACK/redelivery; real lease expiry/recovery; real Playwright BrowserContext execution; real Session revocation/cancellation; real Object Storage Artifact publication; integration; production. Day65's `20 passed` and prior Day59–Day61 evidence are NOT reused as Day66 evidence. No secrets, real credentials, real URLs, Cookies, storage state, Authorization headers, Provider keys, customer data, or raw traces/screenshots/DOM/network payloads committed.

Released — Day65 Browser failure-recovery / security-boundary decision core under `projects/fastapi-playwright/`: `src/day65_recovery_security_policy.py` (timeout classification UNKNOWN_OUTCOME vs SAFE_TO_RETRY; unknown-outcome reconciliation by strict Day64 action identity + server audit (accepted/in-flight != completed: 202 accepted -> CONFIRMED_ACCEPTED_OR_IN_FLIGHT, no replay/publish, keep reconciling same export_id; only completed/imported may publish; only authoritative not_found/never_started may retry); diagnostics policy private/redacted/audited only, never logs/model/prompt/public; navigation/redirect SSRF gate by scheme + exact Origin + resolved IP, blocking loopback/private/link-local/cloud-metadata; scoped credential release with no cross-Origin storage-state forwarding; instruction authority -> PROMPT_INJECTION_BLOCKED; CAPTCHA -> HUMAN_VERIFICATION_REQUIRED; bounded retry eligibility + Retry-After-vs-deadline; incident classification), reusing `day63_session_gate.final_fence`, plus tests and the design/runbook. `python3 -m pytest -q tests/test_day65_recovery_security_policy.py` = 20 passed, EXECUTED_LOCAL_RUNTIME. The LIVE classroom artifact was CONCEPTUAL_STATIC. Core rule: `no observed completion != proven operation failure`; page content is untrusted input, not authorization; a security control is a STOP, not a retry. Day64 is COMPLETE. NOT RUN by the updating agent: real Playwright timeout/reconciliation; trace/screenshot redaction; redirect/DNS/IP policy; storage-state/Cookie behaviour; CAPTCHA handling; audit lookup; a real Worker/queue (Day66); integration; production. Day64's `25 passed` is NOT reused as Day65 evidence. No secrets, real credentials, target URLs, Cookies, tokens, customer data, raw payloads, screenshots, or CAPTCHA-bypass logic committed.

Released — Day64 Dynamic-Extraction / Artifact-Evidence decision core under `projects/fastapi-playwright/`: `src/day64_extraction_contract.py` (task-contract readiness; DOM/network source roles; STRICT network correlation with client_request_id/export_id + safe/redacted metadata; Extraction Contract schema-drift; download/upload validation + precise source/artifact/accepted/rejected counts; Object Storage HEAD verification; retain-candidate/forward-repair on a failed DB reference; the `assemble_trusted_artifact` orchestrator that publishes ONLY if the whole chain passes, reusing `day63_session_gate.final_fence`; and a broad-listener rollback classification) plus `src/day64_controlled_report_page.py` (a synthetic SPA + `/api/reports` + `/api/exports`), tests and the design/runbook. `python3 -m pytest -q tests/test_day64_extraction_contract.py tests/test_day64_report_page_http.py` = 25 passed, EXECUTED_LOCAL_RUNTIME (23 pure decision-core failure-path + 2 controlled report/export page over a REAL HTTP loopback). The LIVE classroom artifact was CONCEPTUAL_STATIC. Core model: `page lifecycle signal != extraction readiness != valid Artifact != published business success`. Day63 is COMPLETE. NOT RUN by the updating agent: real Playwright extraction/network interception/download-upload; the REAL Day61 Object Storage HEAD; a real PostgreSQL Artifact-reference transaction; a real Worker; queue integration (Day66); production. Review fix (v0.1.160): download validation now uses the ACTUAL parsed rows validated against the same TaskContract (DownloadCandidate.parsed_records; no forgeable schema_valid/business_valid booleans; distinct SCHEMA_FIELD_MISSING/TYPE_MISMATCH/VALUE_INVALID/CONTRACT_MISMATCH + BUSINESS_INVALID), so the network JSON and the downloaded artifact are validated independently. Tests 18 -> 25 passed. Review fixes (v0.1.159): export_id can no longer bypass the initial client_request_id match (strict correlate_export + extract_export_id/correlate_followup); the final fence sits at the guarded durable-write boundary (HEAD -> final fence -> guarded txn), so a fence failure commits nothing; the Extraction Contract validates field types/values (FIELD_MISSING/TYPE_MISMATCH/VALUE_INVALID/CONTRACT_MISMATCH); network metadata is an allow-list; and the controlled page is now a genuinely dynamic synthetic SPA for a FUTURE real-Playwright test (still NOT RUN). Tests 16 -> 25 passed. Day63's test results are NOT reused as Day64 evidence. No secrets, real credentials, Cookies, storage-state exports, real target URLs, customer data, raw payloads, or screenshots committed.
...
Target lesson:
Day86 is documented at guided classroom scope and Phase 7B is in progress. Day87 is next; independent Day83–Day86 final syntheses were not assessed.

(Semantics, shared with PROJECT_STATUS.md: "Current Lesson" = the lesson currently being worked on / next up
(Day87); Day86 is finished at guided classroom scope; Day59–70 are complete. Phase 6 (Day67–70) is COMPLETE.
...
- [x] Integrate Day83 exact approval/current checks, interrupt/fence/late evidence, escalation and reservation
  continuation into the evolving `projects/ai-agent/` Artifact; preserve four scenario exercises and actual
  classroom failures/corrections. 53 Day83 / 378 cumulative local tests, 26/26 version-1 seed cases and mandatory checkpoint PASS on Python 3.11.5.
- [x] Preserve17 guided English answers; final Chinese synthesis instructor-authored by request, not independent.
- [x] Prepare the16-section lesson, design, classroom record, cheat sheet/interview and consistent progress links.
- [ ] Independently review Day83 seed expected facts before promoting them to a release gate.
- [ ] Assess independent Day83 final synthesis and validate on Python3.12 (both not yet evidenced).
- [ ] Real Day83 auth/callback/PostgreSQL/Relay/Queue/Worker/Provider/Tool/billing/alert integration (NOT RUN).
- [x] Complete Day84 Conversation Memory vs Durable Business-state Boundaries; do not treat memory as authority.
- [x] Complete Day85 Multi-agent Handoff and Coordination Boundaries; do not treat messages or context as authority.
- [x] Complete Day86 Agent Security without treating Prompt controls, Tool visibility or Sandbox execution as authorization.
- [ ] Start Day87 framework/job-market refresh without pre-locking a framework.
...
- [x] Complete Day82 Durable Agent Jobs, Checkpoint, Resume and Recovery — released the 16-section lesson,
  ...
  and status surfaces. Checkpoints bind Job/Step/Attempt, state version, fence and immutable execution bindings;
  recovery rereads authoritative facts before classification; resume preserves identity, retry requires a new
  authorized Attempt, and unknown execution preserves the original operation and held reservation. The modeled
  transaction writes checkpoint/reservation/audit/Outbox intent facts together; at-least-once redelivery is
  guarded by consumer idempotency and stale results are fenced but retained as evidence. Evidence: CONCEPTUAL +
  STATIC + EXECUTED_LOCAL_RUNTIME (32 Day82 / 325 cumulative tests, Python 3.11.5). Python 3.12, real
  PostgreSQL/Outbox/Relay/Broker/Worker, process crash/restart, multi-process fencing, Provider/Tool/billing,
  INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day81 Agent State Machine, Termination, Loop Detection and Step/Token/Cost Budgets — released the
  16-section lesson, `src/agent_state_machine.py`, 20 deterministic tests, design, classroom record, README,
  cheat sheet/interview and status surfaces. The model only recommends; verified facts and ordered guards create a
  candidate; an authoritative conditional apply checks identity/state/fence and models state plus reservations
  atomically. Unknown external outcomes retain identity and held reservations in `PENDING_RECONCILIATION`;
  known prerequisites `WAIT`; no-progress uses `NO_PROGRESS_LOOP_DETECTED`; rollback retains history and
  compensation is a new operation. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (20 Day81 / 293
  cumulative tests, Python 3.11.5). Python 3.12, real PostgreSQL/Outbox/queue/Provider/Tool/billing,
  INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day78 LLM Application Runtime Capstone, Checkpoint and English Interview (Phase 7A close) — released the 16-section lesson, evolved `projects/ai-agent/` (`src/application_runtime.py`, 22 deterministic Day78 tests, released design and classroom record), README, cheat sheet/interview and status surfaces. The capstone composes existing public seams behind one authoritative lifecycle, immutable bindings, deterministic Provider-request bridge, stable execution envelope, current Tool Admission/execution/outcome verification, guarded completion/reconciliation, separate compensation and idempotent cost settlement. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (22 Day78 / 243 cumulative tests, Python 3.11.5); Python 3.12, real Provider/HTTP/database/queue/Worker/fencing/protected artifact/external Tool/compensation/billing, INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day79 Framework-agnostic Agent Loop and Control Flow (Phase 7B begins) — released the 16-section lesson, `src/agent_loop.py`, 8 deterministic tests, released design, classroom record, README, cheat sheet/interview and status surfaces. The application Controller owns `CONTINUE`/`COMPLETE`/`WAIT`/`FAIL`/`RECONCILE`; model output is advice; terminal/stale/duplicate/non-continue paths create zero Runtime calls; only a new deterministic `CONTINUE` Step composes through the real Day78 preparation boundary. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (8 Day79 / 251 cumulative tests on Python 3.11.5). Durable state/fencing, real Provider/Tool/framework, Day80 governance, Day81 budgets/loop detection, Day82 durability, integration runtime and production NOT RUN. No secrets committed.
- [x] Complete Day80 Tool Registry, Tool Schema and Permission Model — released the 16-section lesson,
  `src/tool_governance.py`, 22 deterministic tests, released design, classroom record, README, cheat
  sheet/interview and status surfaces. A context-scoped immutable Snapshot limits model visibility; Schema
  projection narrows but never authorizes; exact binding plus current Permission/Registry recheck fails closed;
  Framework translation cannot expand authority; Day74 generic and Day66 browser-specific Admission remain
  authoritative; accepted is not completed. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (22 Day80
  / 273 cumulative tests on Python 3.11.5-compatible `python3.11`). Real Provider/Framework/HTTP/PostgreSQL/
  queue/Worker/Playwright, Day81 state machine/budgets, Day82 durability, integration runtime and production
  NOT RUN. No secrets committed.
- [x] Day80 prep: add governed Tool Registry, Schema and Permission Model without bypassing Day74/Day66/Day78 Admission and execution boundaries — completed by the released Day80 Artifact and 22-test suite.

## Completed and Historical Tasks

- [x] Complete Day77 Fake Provider, Contract Tests and LLM Regression Tests (Phase 7A) — released the 16-section lesson, evolved `projects/ai-agent/` (`src/fake_provider_testing.py`, 31 deterministic Day77 tests, released design and classroom record), README, cheat sheet/interview and status surfaces. Preserved the real short answers, the batch missing-result misconception/correction, three English answers and teaching-assistant-supplied final Chinese synthesis attribution. The Fake transport is behind both real Adapters; a controlled gate + FakeClock drive timing; the shared suite asserts stable Provider outcomes; regressions exercise real validation/admission/recovery/guarded completion and independently assert calls/tool effects/Attempts/cost. Ordinary fallback keeps A1 and creates A2; `TIMEOUT_UNKNOWN` keeps A1/no A2/reservation/reconciliation. Independent goldens cannot auto-approve observed output. Bad-v6 recovery separates containment, affected scope, reconciliation, repair, compensation, cost and stale-result fencing. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (31 Day77 / 221 total, Python 3.11.5); no real SDK/HTTP/Provider/database/queue/Worker/tool/billing integration; INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day76 Model Routing, Fallback, Latency and Cost Engineering (Phase 7A) — released the 16-section lesson, evolved `projects/ai-agent/` (`src/routing_policy.py`, `src/recovery_cost.py`, 18 deterministic Day76 tests, released design and classroom record), README, cheat sheet/interview and status surfaces. Preserved the authentic answers, fallback/A1 misconception, p95 correction, latency/batching question, rate-limit uncertainty, incident classifications, three English answers and teaching-assistant-supplied final Chinese synthesis attribution. Core model: eligibility/compatibility before preference; server-owned versioned policy; immutable RoutingDecision/Attempt binding; classified retry/fallback/reject/disable/reconcile; explicit latency boundaries; estimate/reservation/reported/actual/unknown cost. Bad-v5 exercise used rollback for new planning, pause/quarantine/fail-closed containment, per-Attempt classification, repair vs compensation, late-result fencing and guarded cost reconciliation without rewriting history. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (18 Day76 tests; 190 total with 172 Day72–Day75 regressions, Python 3.11.5). No real Provider/HTTP/SSE/database/queue/Worker/tool/live pricing/latency/billing integration; INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day75 Streaming, Caching and Batching for LLM Applications (Phase 7A) — released the 16-section lesson, evolved `projects/ai-agent/` (`src/streaming_cache_batching.py`, 41 deterministic tests, released design and classroom record), README, cheat sheet/interview and status surfaces. Preserved 26 authentic answers, five misconception/correction paths and TA-supplied final Chinese synthesis attribution. Core model: identity/sequence/completion-bound streams emit only a `CompleteCandidate`; exact tenant/auth/input/contract/profile/policy cache hits require freshness/current facts and fresh Day74 Admission; compatible bounded/fair batches retain per-item identity, result and retry/reject/reconcile recovery. Bad-cache-policy exercise used immediate disable/bypass/invalidate, new Attempt instead of rewriting A1, final kill switch for A2, repair plus authorized compensation for A3 and reconciliation for A4. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (41 Day75 deterministic in-process tests; 172 total with 131 Day72–Day74 regressions, Python 3.11.5). No real Provider/SSE/HTTP/Redis/PostgreSQL/queue/Worker/external tool; INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day74 Structured Output, JSON Schema and Function/Tool Calling (Phase 7A) — released the 16-section v2 lesson, the evolved `projects/ai-agent/` Artifact (`src/output_tool_contracts.py` + `tests/test_output_tool_contracts.py` + `docs/DAY74_OUTPUT_TOOL_CONTRACTS.md` + classroom record), README, FastAPI Cheat Sheet/Interview and status surfaces. Preserved 37 real student answers, authentic corrections and the teaching-assistant-supplied final Chinese synthesis attribution. Core model: untrusted candidate -> Parse -> strict Schema -> exact Registry/lifecycle -> trusted-context Authorization -> tenant-scoped Semantics -> immutable per-candidate Admission -> final disable guard + atomic idempotency claim -> Execution -> Outcome Schema/Semantics/Identity -> guarded completion/reconciliation. Bad-v2 A1–A5 exercise distinguished block/new Attempt, kill switch, repair/compensation, TIMEOUT_UNKNOWN reconciliation and safe v1 continuation without rewriting bindings. Evidence: CONCEPTUAL + STATIC (`py_compile` + `mypy`) + EXECUTED_LOCAL_RUNTIME (`python3.11 -m unittest discover -s tests -v` -> 131 tests OK: 34 Day74 + 39 Day73 + 58 Day72, Python 3.11.5). Python 3.12, full JSON Schema compliance, real Provider/SDK/HTTP/DB/queue/external tool, INTEGRATION_RUNTIME and PRODUCTION NOT RUN. No secrets committed.
- [x] Complete Day73 Prompt Contracts, Prompt Versioning and Compatibility (Phase 7A) — released lesson (16-section v2, classroom-loop Concepts + exercises + full English Interview; preserved the real terse student answers, the authentic misconceptions with their corrections, and the Tech-Lead-supplied final Chinese Mental Model authorship note), the evolved `projects/ai-agent/` Artifact (`src/prompt_contracts.py` + `tests/test_prompt_contracts.py` + `docs/DAY73_PROMPT_CONTRACTS.md` released design contract + `docs/day73-prompt-contracts-classroom-draft.md`; README updated), FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/AGENTS/CHANGELOG updates (Day73 Completed; Day74 current). DIRECT PREREQUISITES Day72 (replaceable Provider Adapter) + Day71 (provider-independent runtime) + Day53 (Provider boundary / untrusted success) + Day54 (Attempt lifecycle / timeout-unknown / late-result binding). Core model: the prompt is an application-owned, versioned execution contract (immutable revisions + lifecycle overlay); each Attempt durably binds revision + renderer + parameter policy + application contract + input fingerprint + rendered-message hash; the current default governs new planning only; the LLM Runtime enforces a fail-closed pre-Provider gate (binding mismatch stays PLANNED with 0 calls; disabled bound revision -> BLOCKED_PROMPT_DISABLED with 0 calls); compatibility is directional + structural + semantic; incident recovery = disable/rollback/migration/re-release without rewriting history; TIMEOUT_UNKNOWN -> reconciliation, valid late response interpreted with the BOUND revision. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 97 deterministic in-process tests OK: 39 Day73 + 58 Day72 regression, Python 3.11.5; in-memory stores + pure functions only). INTEGRATION_RUNTIME + PRODUCTION NOT RUN (no real SDK/HTTP/Provider/PostgreSQL/queue/worker/encryption/protected-artifact storage); the provider_calls counter models an in-process boundary crossing only; Day72 tests are prerequisite evidence, not Day73 validation. No secrets committed.
- [x] Complete Day72 Provider Capabilities and the Replaceable Provider Adapter (Phase 7A) — released lesson (16-section v2, classroom-loop Concepts + exercises + full English Interview; preserved the real terse student answers and uncertainty, the English errors with separated language/engineering corrections, and the Tech-Lead-supplied final Chinese Mental Model authorship note), the evolved `projects/ai-agent/` Artifact (`src/provider_contract.py` + `src/provider_adapters.py` + `tests/test_provider_adapters.py` + `docs/DAY72_PROVIDER_ADAPTER.md` released design contract + `docs/day72-provider-adapter-classroom-draft.md`; README updated), FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates (Day72 Completed; Phase 7A In Progress). DIRECT PREREQUISITES Day71 (provider-independent runtime) + Day53 (ProviderRequest -> ProviderOutcome seam) + Day61 (real-HTTP Adapter foundation). Core model: the application owns a stable product contract; Providers offer only versioned capabilities behind a replaceable Adapter (Protocol + Registry) that translates syntax without weakening product meaning; capability admission before a paid call (incompatible -> CAPABILITY_ERROR + 0 calls); immutable versioned Capability Profiles (drift -> disable/quarantine -> new revision); persisted per-Attempt execution contract; Provider SUCCESS untrusted until validation + guarded completion; TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION; rollback stops future harm without erasing history. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 58 deterministic in-process tests OK, Python 3.12.13; RecordingTransport + fixtures only). INTEGRATION_RUNTIME + PRODUCTION NOT RUN (no real SDK/HTTP/Provider/PostgreSQL/credentials/paid call). Provider A/B are fictional fixtures; capabilities are versioned facts; the full Fake Provider/LLM regression suite remains Day77 scope. No secrets committed.
- [x] Complete Day70 n8n + FastAPI + AI Tool Integration Capstone and Interview (Phase 6 close) — lesson (16-section v2, classroom-loop Concepts + exercises + full English Interview; preserved the real short student answers/corrections and the Tech-Lead-taught final synthesis), the capstone artifacts under `projects/n8n-workflows/` (DAY70_CAPSTONE.md + day70_capstone.py + test_day70_capstone.py (21 tests; classroom 14 pre-fix superseded) + Secret-free day70_minimal_acceptance_workflow.json with two independent auth boundaries and a `{document_ids, business_input:{report_scope}}` body), FastAPI/n8n interview + cheat-sheet appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates (Day70 Completed; Phase 6 COMPLETE; Day71 Planned Phase 7A phase-transition). Mixed evidence tiers: a bounded authenticated n8n->FastAPI SERVICE-call acceptance slice INTEGRATION_RUNTIME (in class, PRE-FIX workflow; FIXED workflow NOT RERUN; inbound caller->n8n auth NOT RUN), decision model + workflow static contract EXECUTED_LOCAL_RUNTIME (21 passed; Day59 fingerprint test 12 passed), rest CONCEPTUAL_STATIC / NOT RUN; no post-run n8n export captured; no secrets committed.
- [x] Complete Day71 LLM Application Architecture, Tokens, Context, Sampling and Model Failure Modes (Phase 7A begins) — released lesson (16-section v2, classroom-loop Concepts + design/reasoning exercises + full English Interview; preserved the real terse student answers/questions and corrections and the Tech-Lead-supplied final Chinese synthesis), the `projects/ai-agent/` Phase 7A foundations (`docs/DAY71_FOUNDATIONS.md` released static provider-independent design contract + `docs/day71-llm-runtime-foundations-classroom-draft.md`; README progress updated), FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates (Day71 Completed; Phase 7A In Progress). Day70 -> Day71 is a PHASE_TRANSITION (Phase 6 -> Phase 7A) built on Day53–Day61; n8n is not a prerequisite. Evidence: CONCEPTUAL + STATIC only; EXECUTED_LOCAL_RUNTIME / INTEGRATION_RUNTIME / PRODUCTION are NOT RUN; no runtime code, no real or paid Provider call, no secrets committed.
- [x] Complete Day69 Human Approval, Retry, Secrets, Audit and Error Workflows (Phase 6) — lesson (16-section v2, classroom-loop Concepts + conceptual exercises + full English Interview; preserved the real short student answers, the two-stage event-identity misconception + direct teaching, and the Tech-Lead-taught final Chinese synthesis; kept the To B scenario and risk-based approval), the Day69 Risk-based Approval Gate contract added to `projects/n8n-workflows/README.md`, FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates (incl. the smallest Day70/Day71 wording fix so adjacency isn't misread as dependency). CONCEPTUAL_STATIC only; n8n runtime, FastAPI approval/publication integration, real approval/callback/PostgreSQL/credential-store/Provider behaviours, and production NOT RUN; no exported workflow JSON; Day67's 400 / Day68's contract not reused; no secrets committed.
- [x] Day70 prep: assemble the Phase 6 capstone that DIRECTLY integrates Day69 approval + classified retry/error + Secrets + audit with the real FastAPI backend and permissioned browser/AI tool — completed by Day70 (Phase 6 capstone released; bounded authenticated n8n->FastAPI service-call acceptance slice INTEGRATION_RUNTIME for the PRE-FIX workflow; rest NOT RUN).
- [x] Day70 prep: define the phase-level English interview coverage across Day67–Day69 (boundary, long-job observation/delivery, approval/recovery/audit hardening) — completed by Day70's phase interview.
- [x] Complete Day68 Long-running AI Jobs: Polling, Callback, Correlation and Idempotency (Phase 6) — lesson (16-section v2, classroom-loop Concepts + reproducible-style conceptual exercises + full English Interview; preserved the real short student answers and the one real audit-log correction; noted the final Chinese synthesis was taught directly at the student's request), the Day68 Long-running Job Orchestration Contract added to `projects/n8n-workflows/README.md`, FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates. CONCEPTUAL_STATIC only; n8n runtime, valid FastAPI acceptance/status integration, real polling/callback/PostgreSQL/Worker/Provider behaviours, and production NOT RUN; no exported workflow JSON; Day67's 400 not reused; no secrets committed.
- [x] Day69 prep: design the human-approval gate on top of Day68's stable `task_id`/`correlation_id`/`event_id` — completed by Day69 as a CONCEPTUAL_STATIC contract (Risk-based Approval Gate + exact binding + PENDING-until-backend-expiry lifecycle); the runtime approval endpoint/UI is NOT IMPLEMENTED.
- [x] Day69 prep: specify error/retry workflows that reuse Day68 idempotency — completed by Day69 as a CONCEPTUAL_STATIC classified retry/error matrix (OUTCOME_UNKNOWN + operation_id/idempotency key + 429/503|400/422|401|403|409|terminal|unknown->PENDING_RECONCILIATION) and smallest-safe-boundary Error Workflow; runtime NOT IMPLEMENTED.
- [x] Day69 prep: define Secrets handling and callback authentication for the callback boundary — completed by Day69 as a CONCEPTUAL_STATIC Secret boundary (credential reference vs Credential Store, controlled injection, redaction, 401 stop+classify — rotate only when expiry/compromise/revocation/invalid is established, else fix config); the real credential-store integration and callback authentication are NOT IMPLEMENTED.
- [x] Day69 prep: define the audit contract (correlation_id-linked, backend-authoritative) — completed by Day69 as a CONCEPTUAL_STATIC current-state + append-only approval_events audit contract (atomic commit; append-only not automatically tamper-proof; n8n history != authoritative audit); the real PostgreSQL audit schema is NOT IMPLEMENTED.
- [x] Complete Day67 n8n Workflow Model, Triggers, FastAPI Integration and Responsibility Boundaries (Phase 6) — lesson (16-section v2, preserving the real student answers/misconceptions/corrections + Mental Model Evolution Day66->Day67), `projects/n8n-workflows/README.md` replaced with the Day67 draft-workflow configuration record + ownership boundary + validation matrix + limitations, FastAPI cheat-sheet/interview appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates. DOCUMENTATION + one classroom EXECUTED_LOCAL_RUNTIME 400 proof; valid FastAPI path/service auth/durable Task/PostgreSQL/queue-Outbox/worker execution/published Production URL NOT RUN / NOT CONFIGURED; no exported workflow JSON; no secrets committed.
- [x] Day68 prep: define the FastAPI <-> n8n polling/callback contract (status endpoint + callback shape) — completed by Day68 as a CONCEPTUAL_STATIC contract (Long-running Job Orchestration Contract); the runtime status route/callback endpoint is NOT IMPLEMENTED.
- [x] Day68 prep: specify correlation-ID propagation (request_id/task_id/correlation_id/event_id/task_version/trace_id) across the n8n execution and the FastAPI Job lifecycle — completed by Day68 as the CONCEPTUAL_STATIC identity contract (task_version MODELED / NOT IMPLEMENTED).
- [x] Day68 prep: specify business idempotency for redelivery/retry so a re-sent request or a callback returns the existing task_id and never duplicates work (reuse Day59/Day50 acceptance + the Day67 `request_id` idempotency record) — completed by Day68 as a CONCEPTUAL_STATIC contract (fingerprint-bound event_id + conflict detection); the durable enforcement is NOT IMPLEMENTED.
- [x] Day68 prep: define a scheduled reconciliation idempotency basis (`task_id + recovery_action + recovery_generation`) callable via an authenticated FastAPI reconciliation API (never a direct `browser_tasks` scan) — completed in Day67 (the Day67 reconciliation idempotency basis).
...
- [x] Complete Day47 Async Sessions, Transactions, Repository and Unit of Work (Phase 4) — lesson, async persistence-boundary design + runnable code/fake-session tests (executed: 29 passed; PostgreSQL runtime NOT RUN), FastAPI cheat-sheet/interview appends, and status updates.
...
- [x] Complete Day49 Upload Sessions, Object Storage and Artifact Verification (Phase 4) — lesson, verified-upload-boundary design/runbook + runnable provider-neutral fake-adapter model/tests (executed: 44 passed after review rounds 1-2, application control flow only; real PostgreSQL/Object Storage/integration/production NOT RUN), FastAPI cheat-sheet/interview appends, README increment, and status updates.
...
- [x] Complete Day51 Authentication, Password Security and JWT (Phase 4) — lesson (16-section v2, verbatim student answers + labeled assistant-assisted synthesis), auth design/runbook + runnable provider-neutral REAL-crypto model/tests (executed: 37 passed with real Argon2id + real RS256 JWT ephemeral keys; real PostgreSQL/FastAPI/browser/JWKS/integration/production NOT RUN; JWE out of scope; no exactly-once/authorization claims; schema honesty: password_hash + AuthSession modeled, real schema needs a Day48-safe additive migration), FastAPI cheat-sheet/interview appends, README increment, and status updates.
- [x] Day51 review (Codex) fixes — F1 grace retry now recovers the SAME usable token B once from a short-TTL encrypted recovery slot (raw never plain-persisted/logged); F2 per-family used-hash ledger detects replay of ANY earlier token (A->B->C then replay A) -> revoke+retain family, isolate other devices; F3 `revoke_key` blocks signing and revoking the current signer fails closed until K2 is promoted; F4 secure Argon2id production default (weak params test-only); F5 requirements pinned to executed versions + PROJECT_STATUS/TASKS Current/Next semantics unified. Tests 27 -> 34 passed; full api suite 238 passed. Real PostgreSQL/FastAPI/browser/JWKS/integration/production still NOT RUN.
...
- [x] Complete Day54 AI Streaming, Client Disconnects, Timeouts and Cancellation (Phase 4) — lesson (16-section v2, verbatim CN/EN student answers + four correction trajectories + labeled assistant-assisted final Chinese summary), streaming/lifecycles/cancellation design/runbook + runnable provider-neutral standard-library-only in-memory model/tests (executed: 27 passed; real FastAPI/SSE wire, real OpenAI SDK/network/Provider token stream, real PostgreSQL/Redis/Celery, integration, production NOT RUN; three independent lifecycles; disconnect ends only a subscription; timeout is non-terminal PENDING_RECONCILIATION; durable+auditable+cooperative+guarded cancellation/expiry; one guarded winner; crash-safe re-observation; late result after terminal refused; erroneous disconnect->cancel rollout recovered by policy rollback + evidence-based reconciliation; schema honesty: cancelled/expired/pending_reconciliation + intent table modeled, real schema needs a Day48-safe additive migration), FastAPI cheat-sheet/interview appends, requirements-day54.txt, README increment, and status updates.
- [x] Complete Day55 Celery, Worker Execution and Long-running AI Jobs (Phase 4) — lesson (16-section v2, verbatim CN/EN student answers + six misconception corrections + labeled assistant-assisted final Chinese Mental Model), Celery Worker execution/recovery design/runbook + runnable provider-neutral standard-library-only in-memory model/tests (executed: 27 passed; full api suite 375; a real Celery broker/Worker, real ACK/redelivery/visibility-timeout, Worker-loss/OOM fault injection, real PostgreSQL/Redis, and the real Provider NOT RUN; guarded claim = first duplicate-call gate (lease/fencing secondary); ACK/SUCCESS != Job succeeded; timeout/OOM -> PENDING_RECONCILIATION with no blind re-call; envelope-poison-before-load vs contract-poison-after-load, transient -> bounded retry (Day56 depth); Day54 durable cancellation preserved (intent first, revoke best-effort, one guarded winner); Outbox publish-before-checkpoint; graceful drain; erroneous early-ACK release recovered by policy rollback + evidence-based reconcile, no bulk flip; Day40 semantics on a SUPPORTED Celery broker, not the custom Streams design; schema honesty: statuses + intent table + attempt fields modeled, real schema needs a Day48-safe additive migration), FastAPI cheat-sheet/interview appends, requirements-day55.txt, README increment, and status updates.
- [x] Day55 review (Codex) fixes — F1 lease-expiry/redelivery re-claim with persisted provider_request_id -> RECONCILE_ONLY (Job -> PENDING_RECONCILIATION, zero Provider re-calls; lease expiry != external-execution re-authorization); F2 a durable cancellation intent persisted after the RUNNING guarded claim is caught by a post-claim, pre-Provider re-check (zero Provider calls, guarded terminal CANCELLED/EXPIRED) in addition to the QUEUED pre-claim path and the final pre-completion re-check; F3 provider_request_id_recorded attributed to the real parent Job via a durable attempt_id->job_id path with attempt_id/provider_request_id/correlation_id repair evidence (never correlation_id as a Job id); F4 build_affected_set bounded by release + a running_since time window + auditable running evidence (out-of-window/no-evidence same-release running Jobs excluded); F5 published invariant celery_task_id == job_id (OutboxRelay stamps + asserts it; celery_task_id_for_job) so revoke targets the correct Celery task, durable intent still the sole business authority. Added 9 regression tests; tests 27 -> 36 passed; full api suite 384. Real Celery broker/Worker, ACK/redelivery/visibility-timeout, Worker-loss/OOM fault injection, real PostgreSQL/Redis, and the real Provider still NOT RUN.
- [x] Day55 review (Codex) P1 recovery-gap fix — conservative durable marker (provider_dispatch_started_at) persisted BEFORE the Provider request leaves the process (order: guarded claim -> marker -> Provider call -> record provider_request_id -> validate/terminal); a lease-expired redelivery reconciles when the Attempt has EITHER a provider_request_id (strong evidence) OR the marker (conservative evidence), closing the OOM-after-dispatch-before-request-id gap (missing id != Provider not executed); accepted safety-first false positive (marker set, request maybe not sent) -> reconcile, never retry; provider idempotency key reduces risk but is not a safe-retry license. classify_repair treats the marker as evidence too. Added 4 regression tests; tests 36 -> 40 passed; full api suite 388. Real Celery broker/Worker, ACK/redelivery/visibility-timeout, Worker-loss/OOM fault injection, real PostgreSQL/Redis, and the real Provider still NOT RUN.
- [x] Complete Day56 Provider Resilience, Rate Limits, Token Cost and Backpressure (Phase 4) — lesson (16-section v2, verbatim CN/EN student answers + corrections + labeled assistant-assisted final Chinese Mental Model), admission-to-Provider control-plane design/runbook + runnable provider-neutral standard-library-only in-memory model/tests (executed: 31 passed; full api suite 419; a real Celery broker/Worker, a real Redis distributed limiter/circuit, real PostgreSQL, real Provider traffic/rate limits/costs, load, and Worker-kill fault injection NOT RUN; four authorities guarded-claim/rate-permit/reservation/circuit; five outcomes CALL/DEFER/RECONCILE/TERMINAL/NOOP; bounded backoff+jitter, Retry-After earliest floor, retry storm != cache avalanche; no-permit -> durable DEFER, zero execution-retry, bounded by deadline; limiter outage fail-closed; worst-case reservation + settle/release to tenant ledger; backpressure before 202, tenant 429 vs system 503; no silent contract mutation; 429 by execution certainty, unknown -> reconcile; circuit OPEN/HALF_OPEN progressive probes; deadline release only w/o evidence; zero-defer incident rollback + bounded evidence + new Outbox intent, evidence -> RECONCILE_ONLY; schema honesty: deferred status + defer record + reservation ledger modeled, real schema needs a Day48-safe additive migration, limiter/circuit transient), FastAPI cheat-sheet/interview appends, requirements-day56.txt, README increment, and status updates.
- [x] Complete Day57 AI Backend Testing, Fake Providers, Contract Tests and Failure Injection (Phase 4) — lesson (16-section v2, verbatim CN/EN student answers + corrections + labeled assistant-assisted final Chinese mental model), verification design/runbook + runnable deterministic harness/tests (executed: 21 passed; full api suite 463; standard-library control flow driving the REAL Day56 policy functions + Day53's real pydantic validator). Turns the Day43–Day56 policies into repeatable evidence and injects failures with THREE explicit evidence tiers: controllable Fake Provider (scripted outcomes, independent call log surviving Worker loss, request_received/release_response gates), FakeClock + DeterministicRandom, application-owned ProviderAdapter/ProviderOutcome (no SDK leakage, never writes Job/cost), strict attempt_late_completion, VALIDATION_MATRIX/not_run_claims(). Scenarios: bare-429 -> PENDING_RECONCILIATION + call count one + no new permit + reservation HELD; missing request id != no execution (Day55 dispatch marker forces RECONCILE); valid-JSON schema violation = CONTRACT_VIOLATION; deterministic backoff Retry-After floor (no wake-all); controlled timeout window no sleeps; late-result strict identity+schema match, terminal CANCELLED rejects; limiter outage fail-closed; deadline with/without evidence; admission 503>429; guarded idempotent repair (unique repair_id -> one Outbox intent, provider-evidence RECONCILE_ONLY). NOT RUN: real PostgreSQL/Celery/broker/Worker-kill/Redis/Provider integration (marked in the matrix); job_repair_history table/migration is forward-additive design only; pytest passed != audit evidence. FastAPI cheat-sheet/interview appends, requirements-day57.txt, README increment, and status updates.
- [x] Complete Day58 Production AI API Capstone, Observability and English Interview (Phase 4 capstone) — lesson (16-section v2, verbatim CN/EN student answers + corrections + mental-model evolution + labeled assistant-assisted final Chinese mental model), observability capstone design/runbook + runnable deterministic in-process model/tests (executed: 21 passed; full api suite 486; final after review rounds (through the strict-bool fix, commit cde890c): 38 passed, full api suite 503; standard-library control flow, imports Day57 EvidenceTier + Day56 ExecutionCertainty). Makes distributed execution explainable/auditable around durable state: five-identity lifecycle (job_id/correlation_id stable; attempt_id/trace_id per Attempt; request_id per request); safe StructuredEvent contract (no raw prompts/responses/secrets; timeout vs suppressed reason=prior_attempt_may_have_executed); low-cardinality MetricRegistry (Counter/Gauge/Histogram; rejects job_id/attempt_id/trace_id labels); trace/span-link modeling (child span shares trace, async link to immediate prior); telemetry-exporter-failure policy (keep core processing, never FAIL a Job, health metrics); bad-observability-release rollback drill (config not DB facts, bounded set, mark gaps, marker-backed = reconcile-only). NOT RUN: real FastAPI+OpenTelemetry exporter, real PostgreSQL/Redis/Celery integration, real Provider traffic (INTEGRATION_RUNTIME + PRODUCTION per VALIDATION_MATRIX_DAY58); observability is evidence around durable state, never a retry authority. FastAPI cheat-sheet/interview appends, requirements-day58.txt, README increment, and status updates. Phase 4 COMPLETE as classroom scope + deterministic EXECUTED_LOCAL_RUNTIME artifacts (real FastAPI/OpenTelemetry/PostgreSQL/Redis/Celery integration + Provider production NOT RUN).
- [x] Complete Day59 Real FastAPI Runtime, PostgreSQL and Alembic Integration (Phase 5; Production Integration Gate) — lesson (16-section v2, real short CN/EN student answers + reasonable errors + corrections + Mental Model Evolution + labeled assistant-assisted final Chinese mental model), design/runbook + local-only FastAPI composition `day59_runtime_app.py` (`/livez`, `/readyz` gated on expected Alembic revision, POST `/v1/jobs` atomic acceptance, tenant-scoped GET) reusing Day47 async boundaries + pure stdlib `day59_acceptance_logic.py` (request fingerprint, idempotency classification, readiness gate) with `test_day59_acceptance_logic.py` (executed by the updating agent: 10 passed, EXECUTED_LOCAL_RUNTIME) + forward additive `0008_day59_acceptance` migration (nullable request_fingerprint + SHA-256-shape CHECK + partial unique index one dispatch intent/Job) + controlled ONLINE-only `env.py` version-table width repair + `requirements-day59.txt`. Core rule: 202 = ONE committed acceptance bundle (queued Job + fingerprint + one job.dispatch_requested Outbox intent + Document links) verified from a NEW connection; the HTTP transaction never calls a Broker/Worker/Provider/Object Storage. Real local INTEGRATION_RUNTIME executed in a disposable local env during class (Python 3.11; Uvicorn + PostgreSQL 16; raw Day42 baseline -> Alembic stamp -> upgrade through 0008; readiness/atomic-accept/replay/409/422/concurrent-same-key with independent fresh-connection 1/1/1 & 0/0/0 queries; a prior version-table-width failure + an async SQL param-type failure were diagnosed and fixed). Day59 REVIEW FIX then corrected the acceptance path — P0 single `session.begin()` + `INSERT ... ON CONFLICT DO NOTHING RETURNING` create-or-return (no autobegin-then-begin); P0 real Document verification via `app.documents` JOIN `app.upload_sessions` `session_status='verified'` (no `documents.verified_at`); P1 fingerprint covers ordered `document_ids` (same key + different Document -> 409) and excludes the Idempotency-Key per the Day50 contract; P1 `Idempotency-Key` header (400 on missing/blank), non-empty `document_ids`; P1 conflict re-read (replay vs 409, no swallowed IntegrityError); requirements add greenlet + psycopg2 (sync Alembic driver); lesson typo fixed. Pure-logic tests 7 -> 12 passed. The corrected acceptance path's real PostgreSQL INTEGRATION_RUNTIME was NOT RERUN by the updating agent (no Docker/PostgreSQL available). Day59 REVIEW ROUND 2 — P1 persist Document input order (`document_role='input'`, `input_order=1..n`, client order preserved, no set()/dict.fromkeys()); P1 reject duplicate `document_ids` with 422 before the transaction (no Job/Outbox/links); P1 corrected runtime-evidence claims (removed `same key + different Document -> 409` and `concurrent same key + different payload -> 202/409` from executed-in-class; moved to a Required integration rerun matrix, NOT RERUN). Pure-logic tests 10 -> 12 passed; INTEGRATION_RUNTIME still NOT RERUN. NOT RUN: real Redis/Celery broker/Relay/Worker + worker-kill/redelivery (Day60); real Object Storage/Provider HTTP/OpenTelemetry exporter (Day61); real JWT/JWKS/secret manager; zero-downtime migration; multi-replica; load; production. `pytest passed` never auto-upgrades to INTEGRATION_RUNTIME/PRODUCTION. No secrets/local URLs/passwords/tokens/tenant fixtures/.venv/container IDs committed. FastAPI cheat-sheet/interview appends, README increment, and status updates.
- [x] Complete Day60 Outbox, Redis/Celery Broker and Worker Recovery Integration (Phase 5; Production Integration Gate) — lesson (16-section v2, real CN/EN student answers + corrections + Mental Model Evolution), design/runbook + pure stdlib `day60_delivery_recovery_logic.py` (relay publish-before-checkpoint, guarded-claim outcome, duplicate/redelivery/expiry classification, recovery-sweep result, bounded early-ACK repair eligibility + idempotent `repair_id`, readiness gate) with `test_day60_delivery_recovery_logic.py` (executed by the updating agent: 11 passed, EXECUTED_LOCAL_RUNTIME) + readiness app-factory `day60_runtime_app.py` (`create_app(expected_revision='0009_day60_delivery_runtime')`, explicit param) + `day60_celery_config.py` (acks_late/reject_on_worker_lost/prefetch1) + forward additive `0009_day60_delivery_runtime` migration (outbox relay_owner/relay_token/relay_claim_expiry + `job_repair_history`) + `requirements-day60.txt`. Core: Relay publishes then checkpoints `published_at` (at-least-once; NULL = no checkpoint, not no-execution); guarded Worker claim + lease fencing (stale Worker can't commit); late ACK != commit; expired lease + evidence -> PENDING_RECONCILIATION else sweep `running->queued` + one `job.redispatch_requested` intent; bounded early-ACK repair, idempotent `repair_id`, never `.delay()`. Day60 REVIEW FIX — added a REAL runtime `day60_delivery_runtime.py` (OutboxRelay FOR UPDATE SKIP LOCKED claim + publish-outside-lock + fenced checkpoint; guarded queued->running Worker claim + Attempt/Event + lease-token guarded completion; recovery_sweep only on expired running -> evidence=pending_reconciliation else running->queued+audit+one redispatch; repair_early_ack release-filtered, immutable audit linked to one redispatch intent, never .delay()/apply_async()) + forward-additive `0010_day60_runtime_schema` (jobs lease/marker/release_version cols, status widened with pending_reconciliation, job_repair_history.redispatch_outbox_event_id UNIQUE). Pure-logic fixes: queued/terminal never swept; sweep only expired running; release-filtered repair; unified lease boundary (>now active, <=now expired). Pure-logic tests 11 -> 25 passed. Day60 REVIEW ROUND 2 — P0 unified the runtime to the EXISTING Day48 lease TRIPLE lease_owner/lease_token/lease_expires_at (0011_day60_lease_realign drops the parallel lease_expiry 0010 added; guarded claim writes all three atomically, satisfies jobs_lease_triple_coherent + jobs_running_requires_lease; completion matches lease_token not owner); P0 added a REAL Celery app day60_celery_app.py + Relay/sweeper entrypoints day60_relay.py/day60_sweeper.py (only the Relay publishes via apply_async); P1 guarded completion keeps Job/Attempt/Event/lease consistent (succeeded + attempt_count++ + Attempt.finished_at + success Event + cleared triple in one tx; lost lease writes nothing/keeps Attempt open; sweep preserves old Attempt, new attempt_number); P1 bounded the repair set (real incident window via in_time_window, caller attestations for no-conflict/deadline-contract-budget); P2 runbook upgrade/readiness -> 0011 + real Redis/Relay/Worker/sweeper + fault-injection commands. Tests 18 -> 26 passed. Day60 REVIEW ROUND 3 — P1 persist repair incident window + operator attestation (0012_day60_repair_audit_attestation adds nullable incident_start/incident_end/no_conflict_attested/deadline_contract_budget_valid_attested to job_repair_history; repair_early_ack writes them in the repair tx); P1 IntegrityError handler re-reads the committed repair_id row in a fresh tx and returns already_applied ONLY for a matching same-repair duplicate else repair_failed (classify_repair_integrity + tests) — no faked success; P2 corrected the 0011 wording (controlled corrective DESTRUCTIVE drop of a never-written teaching column, NOT additive/zero-downtime; future production guidance added) and made the requirements/runbook test commands copy-pasteable running both Day60 test files. Head/readiness -> 0012. Tests 26 -> 31 passed. Day60 REVIEW ROUND 4 — P1 IntegrityError duplicate check now verifies linked-Outbox semantics (fresh re-read JOINs job_repair_history to outbox_events; already_applied only if the linked Outbox exists, its job_id equals this Job, and event_type='job.redispatch_requested', plus repair job/release/reason match; a non-null FK to a foreign/wrong-type Outbox is repair_failed); RepairFact gained linked_outbox_job_matches + linked_outbox_is_redispatch; added wrong-job/wrong-event-type negative tests + a static join contract check. P2 fixed the last runbook '(0011)' head wording -> 0012. Tests 31 -> 34 passed. INTEGRATION_RUNTIME NOT RERUN (no Docker/PostgreSQL/Redis/Celery). NOT RUN: real Provider HTTP/Object Storage/OpenTelemetry (Day61); Playwright (Day62); production load/security/zero-downtime/scheduling/multi-replica. `pytest passed` never auto-upgrades to INTEGRATION_RUNTIME/PRODUCTION. No secrets/local URLs/passwords/tokens/fixture ids/.venv/container ids committed. FastAPI cheat-sheet/interview appends, README increment, and status updates.
- [x] Day56 review (Codex) P1 fixes — P1-1 Retry-After earliest floor + bounded jitter above it (no wake-all); P1-2 HALF_OPEN probe slot consumed only at CALL (no leak/strand on DEFER); P1-3 worst-case cost covers bounded input+output with separate prices + protected over-reservation policy (OVERAGE_RECONCILE, never overdraw) + idempotent reserve; P1-4 guarded idempotent repair_redispatch (stable repair_id -> one Outbox intent under duplicate/concurrent repair; re-checks affected-set/EXPIRED/cancel/deadline/provider-evidence/budget; audited EXPIRED history; Provider evidence -> RECONCILE_ONLY). Added 12 regression tests; tests 31 -> 43 passed; full api suite 431. Real Celery/Redis/PostgreSQL/Provider/load/fault-injection still NOT RUN.
- [x] Day56 review (Codex) concurrency P1 fixes — P1-1 atomic lock-guarded try_acquire_probe for HALF_OPEN (only the slot winner CALLs; loser releases its rate permit and DEFERs; no leak / no probe over half_open_max_probes); P1-2 repair_redispatch wraps repair-id claim + eligibility + reservation + audit + status change + single Outbox intent in ONE lock-guarded critical section (two concurrent same-id repairs -> exactly one REDISPATCHED + one ALREADY_APPLIED, one Outbox intent, one reservation). In-memory locks model the atomic boundary (real: DB row lock / ON CONFLICT / FOR UPDATE, or Redis Lua). Added 2 threading.Barrier concurrency tests; tests 43 -> 45 passed; full api suite 433. In-memory concurrency only — NOT PostgreSQL isolation / real Redis / Celery / production.
- [x] Day56 review (Codex) concurrency P1 fix — TenantBudgetLedger reservation/balance ops now run under a ledger-level lock (reserve_worst_case/has_reservation/settle_actual/release_reservation/hold_for_reconciliation/available/can_afford), making affordability check + deduction + reservation write ONE atomic critical section so two Jobs racing a single-Job tenant balance can never both reserve/overspend; reserve_worst_case idempotent per job_id. Existing semantics preserved. In-memory lock models the atomic boundary (real: UPDATE ... WHERE available - reserved >= :amt RETURNING / SELECT ... FOR UPDATE). Added 3 threading.Barrier/idempotency regression tests; tests 45 -> 48 passed; full api suite 436. In-memory concurrency only — NOT PostgreSQL isolation / real Redis / Celery / production.
- [x] Day56 review (Codex) concurrency P1 fix — CircuitBreaker guards all per-failure-domain state (_state/_fails/_probes_in_flight/_probe_successes) under one threading.RLock: concurrent record_failure never loses a count and reliably OPENs at the threshold; concurrent HALF_OPEN probe success/failure keep in-flight + state consistent. RLock avoids nested-lock deadlock; has_probe_capacity stays a hint, try_acquire_probe the authoritative atomic acquire, loser releases rate permit + DEFERs, progressive OPEN/HALF_OPEN/CLOSED recovery preserved. Added 3 threading.Barrier concurrency-state tests; tests 48 -> 51 passed; full api suite 439. In-memory concurrency only — NOT Redis Lua / PostgreSQL isolation / Celery / production.
- [x] Day56 review (Codex) concurrency P1 fix — record_probe_success is HALF_OPEN-gated: a late success returning after a probe failure re-OPENed the circuit releases its slot but never counts, never re-CLOSEs a known failure, and never carries into the next HALF_OPEN round (a failed probe latches OPEN). RLock boundary kept; fail-reopens / try_acquire_probe / progressive close unchanged. Added 3 tests (controlled-order + concurrent); tests 51 -> 54 passed; full api suite 442. In-memory concurrency only — NOT Redis Lua / PostgreSQL isolation / Celery / production.
- [x] Day54 review (Codex) fixes — P1-1 deadline intent -> guarded terminal EXPIRED (user cancel -> CANCELLED) via terminal_for_intent, consistent across pre-call/mid-stream/pre-completion/crash re-observation; P1-2 a durable intent written after the last token but before completion is caught by a FINAL cooperative check (no succeeded; guarded cancel/expiry); P1-3 provider_request_id persisted at Provider-request open (mid-stream cancel/timeout -> RECONCILE_UNKNOWN_EXTERNAL); P1-4 late result reuses Day53 identity binding (job_id + attempt_id + correlation_id + provider_request_id) + strict validation gate before any Artifact, side-effect-free on mismatch/missing/not-awaiting/terminal/invalid, at-most-once on duplicate/concurrent, zero Provider calls; P2 fixed the request_cancellation default kind. Tests 15 -> 27 passed; full api suite 348 passed. Real FastAPI/SSE, OpenAI SDK/network/Provider, PostgreSQL/Redis/Celery, integration, production still NOT RUN.
- [x] Day53 review (Codex) fixes — (1) the outgoing Provider call is now bound to the persisted ExecutionContract (bind_request_to_contract; tampered model/schema/version rejected as CONTRACT_MISMATCH before any transport call; max tightened, never enlarged); (2) Provider timeout -> non-terminal PENDING_RECONCILIATION (not terminal FAILED) so a matching late result completes via guarded completion, reservation retained, no auto second call; (3) known-usage non-success paths settle the exact usage via record_cost (validation-fail/refusal/incomplete; refusal carries usage; unknown -> reconciliation_pending); (4) config-wide capability 400 fails the ProviderConfig closed (blocks next call) while a single-request 400 does not. Tests 20 -> 33 passed; full api suite 306 passed. Real openai SDK/network/Provider, PostgreSQL/Redis/Celery/FastAPI wire/integration/production still NOT RUN.
- [x] Day53 review (Codex) fix — pre-call execution gate + late-outcome ingestion: `execute_job` now claims execution eligibility BEFORE any `adapter.generate()` (only a RUNNING Job may start a paid call; a SUCCEEDED/FAILED/PENDING_RECONCILIATION Job re-execute is PRECALL_BLOCKED with transport calls == 0, so a terminal/pending Job never re-triggers a paid Provider call just to be NOOP'd at the DB write). A result arriving after a timeout is handled by a new no-adapter `ingest_late_outcome(outcome, job_id, correlation_id)` path (validate job_id + correlation against the persisted contract -> guarded completion; mismatch -> LATE_OUTCOME_REJECTED; terminal -> COMPLETION_NOOP), never a second execute_job. Tests 33 -> 36 passed; full api suite 309 passed. Real openai SDK/network/Provider, PostgreSQL/Redis/Celery/FastAPI wire/integration/production still NOT RUN.
- [x] Day53 review (Codex) P1 fixes — (1) ATOMIC pre-call claim: `claim_for_new_call` (lock-guarded, models `UPDATE ... WHERE status='running' AND open_attempt IS NULL RETURNING`) creates exactly one IN_FLIGHT `Attempt`; two concurrent/re-entrant `execute_job` calls -> at most ONE calls transport, the loser is PRECALL_BLOCKED (claim_conflict) with 0 calls. (2) Path B `ingest_late_outcome` now validates the PERSISTED `Attempt` (attempt_id + correlation + provider_request_id; correlation alone is not proof; a request id absent at send time is first-recorded on arrival) and rejects wrong-attempt/correlation/request-id as LATE_OUTCOME_REJECTED. (3) ANY late outcome on a terminal (SUCCEEDED/FAILED) Job is a guarded COMPLETION_NOOP — invalid late success / refusal / incomplete no longer add Events or overwrite settled_tokens/cost_state/result. Adapter passes safe correlation metadata; timeout preserves the request id on the Attempt. Tests 36 -> 41 passed (incl. a concurrency test); full api suite 314 passed. Real openai SDK/network/Provider, PostgreSQL/Redis/Celery/FastAPI wire/integration/production still NOT RUN.
- [x] Day53 review (Codex) P1 fix — Path B late-outcome consumption is concurrency-safe + idempotent: added `AttemptStatus.PROCESSING_LATE_OUTCOME`/`CONSUMED` + a lock-guarded atomic `claim_late_outcome` (validates job/attempt/correlation/provider_request_id AND flips `AWAITING_LATE_OUTCOME -> PROCESSING_LATE_OUTCOME` in ONE critical section). Only the single winner runs `_dispatch_outcome`; a terminal Job or a concurrent/duplicate delivery returns `COMPLETION_NOOP` WITHOUT dispatch (no Event/cost/result write); after dispatch the Attempt is `CONSUMED` (a dispatch exception releases it back to `AWAITING_LATE_OUTCOME`, never a Provider re-call). At-least-once redelivery is consumed at most once. Tests 41 -> 45 passed (incl. a concurrent-duplicate-refusal test); full api suite 318 passed. Real openai SDK/network/Provider, PostgreSQL/Redis/Celery/FastAPI wire/integration/production still NOT RUN.
- [x] Day53 review (Codex) P1 fixes — (1) strict provider_request_id in `claim_late_outcome`: a RECORDED id now requires a present + EQUAL incoming id (a MISSING incoming id is rejected, not accepted), and no-recorded-id + missing-incoming is rejected (a controlled first-record needs a non-empty id) -> LATE_OUTCOME_REJECTED with no Event/cost/result/status/Attempt change. (2) Path B dispatch is one UoW: snapshot pre-dispatch Job facts -> dispatch -> CONSUMED on success; on a dispatch exception (e.g. cost-write failure) roll ALL partial writes (Event/status/cost/settled_tokens/result) back to the snapshot THEN reopen the Attempt to AWAITING_LATE_OUTCOME, so a later legitimate redelivery yields exactly one complete result; never a Provider re-call. Tests 45 -> 48 passed (incl. missing-request-id reject + injected cost-write-failure rollback); full api suite 321 passed. Real openai SDK/network/Provider, PostgreSQL/Redis/Celery/FastAPI wire/integration/production still NOT RUN.
- [x] Prepare for Day52 — Authorization, Tenant Isolation, Quotas and API Security (Phase 4) — completed by Day52 (see the Day52 completion entry above).
...
- [x] Run `py_compile` and `pytest` on the Day48 artifact and record exact results (44 static/offline tests passed) plus an offline `alembic upgrade --sql` DDL render; label PostgreSQL runtime NOT RUN; do not use SQLite/fake-session/`upgrade`-success as PostgreSQL evidence.
...
- [x] Run `py_compile` and `pytest` on the Day47 code and record exact results (23 fake-session control-flow tests passed); label PostgreSQL runtime NOT RUN; do not use a mock or SQLite as PostgreSQL evidence.
...
- [x] Update `docs/README.md` (Day47 is the latest FastAPI lesson).
- [x] Update the Day46 lesson Next Lesson link to the released Day47 lesson.
- [x] Update `CURRICULUM.md` (Day47 Completed; Phase 4 In Progress; Day48-Day100 remain Planned) and `ROADMAP.md` (Day47 Completed).
- [x] Update `PROJECT_STATUS.md`, `TASKS.md`, `CHANGELOG.md`, and `AGENTS.md`.
...
- [x] Build and RUN the runnable artifact + tests (Pydantic 2.5.0, pytest -> 24 passed); record honest evidence.
...
- [x] Run `py_compile` and `pytest` on the Day44 code and record exact results (24 passed) as repository evidence.
...
### Day47 Preparation — Async Sessions, Transactions, Repository and Unit of Work (completed)

- [x] Read the Day47 input when provided.
- [x] Review the Day46 SQLAlchemy mapping and the Day45 process-vs-request scope boundary that Day47 drives.
- [x] Preview an app/process-scoped AsyncEngine and a request/Job-scoped AsyncSession, transaction boundaries, a repository, and a unit of work over the Day33 guarantees.
- [x] Keep Alembic migrations (Day48), Outbox acceptance (Day50), and real Provider integration (Day53) out of scope.
...
- [x] Built Pydantic v2 validation, typed request/response models, structured AI output contracts, and error shapes; ran the tests (24 passed).
