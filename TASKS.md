...
(Semantics, shared with PROJECT_STATUS.md: "Current Lesson" = the lesson currently being worked on / next up (Day74);
...
- [ ] Day74 prep (Phase 7A): build Structured Output, JSON Schema and Function/Tool Calling on the Day73 versioned prompt boundary — constrain and validate the model OUTPUT (reusing Day44 Pydantic contracts) while preserving that transport `SUCCESS` is an untrusted candidate. Continues the same `projects/ai-agent/` Artifact; do not pre-implement Day75–Day78.
