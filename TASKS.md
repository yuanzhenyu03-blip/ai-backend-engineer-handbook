...
(Semantics, shared with PROJECT_STATUS.md: "Current Lesson" = the lesson currently being worked on / next up (Day72);
...
- [ ] Day72 prep (Phase 7A): build Provider Capabilities and the Replaceable Provider Adapter on the Day71 provider-independent contract — hide provider capability differences behind one stable, replaceable Adapter that never silently weakens the application contract (extends Day53). Continues the same `projects/ai-agent/` Artifact; do not pre-implement Day73–Day78.
