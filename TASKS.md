...
- [x] Keep model/Framework output as an application-owned proposal with zero authorization/dispatch authority.
- [x] Compose Tool governance, human approval, authentication/current authorization and exact permits.
- [x] Preserve stable operation identity and fresh attempt/request/generation identity across retries/restarts.
- [x] Persist an atomic `DISPATCH_STARTED` claim before transport; allow exactly one dispatch winner.
- [x] Require candidate correlation, protocol validation, output validation and the sole Committer.
- [x] Preserve possibly executed timeout and graceful shutdown as `PENDING_RECONCILIATION`.
- [x] Recover in a separate process through read-only authority query without Tool replay.
- [x] Keep `NOT_FOUND` pending and gate retry behind committed authoritative `NOT_EXECUTED`.
- [x] Separate credential-safe logs, bounded metrics and traces from authorization/durable authority.
- [x] Run 38 focused dependency-free tests, one grader, three SDK/restart integration tests and 16/16 seed.
- [x] Run 761 total tests plus all available Day83–Day94 seeds and the deterministic example.
- [x] Complete the Phase 7B English Interview and independent final synthesis with corrections.
- [x] Release the lesson, design, classroom record, cheat sheet/interview, evidence and Day95 handoff.
- [x] Record `RESTART_RECOVERY_RUNTIME` and `MORE_EVIDENCE_NEEDED`.

Day93 completion record:

- [x] Separate one absolute deadline, phase timeout, timeout budget and cancellation semantics.
- [x] Add application-owned failure evidence with independent dispatch and execution certainty.
- [x] Add bounded retry policy, exponential deterministic jitter and conditional single-worker dispatch claim.
- [x] Add read-only reconciliation, late-response validation, bounded scheduling and operational alerts.
- [x] Add generation-scoped reconnect correlation, version negotiation and downgrade-safe capability permits.
- [x] Add fail-closed JWKS refresh, global/tenant capacity boundaries, circuit evidence and safe observability.
- [x] Run 62 focused tests, including real SDK independent-process loopback Streamable HTTP success/timeout.
- [x] Run 719 combined tests, 16/16 Day93 seed and available Day83–Day93 seed suites on Python 3.11.5.
- [x] Release lesson, cheat sheet, interview material, Decision 014, example, evidence and Day94 handoff.
- [x] Record `CONTROLLED_REMOTE_RUNTIME`, assessed learner synthesis and `MORE_EVIDENCE_NEEDED`.

Day92 completion record:

- [x] Separate HTTP per-request Bearer identity from stdio trusted-Launcher identity.
- [x] Add minimized principal and deterministic signature/issuer/audience/time/subject validation.
- [x] Add current revocation, scope, tenant membership and exact Tool/Resource/Prompt permits.
- [x] Reject payload/URI/Prompt/capability privilege expansion and preserve enumeration resistance.
- [x] Preserve operation identity, candidate-only handlers, safe retry and pending reconciliation.
- [x] Add 35 focused tests, 16 seed cases, deterministic example, lesson, design, classroom record and Day93 handoff.
- [x] Run 656 combined tests and available Day83–Day92 seed suites on Python 3.11.5.
- [x] Record `INTEGRATION_RUNTIME`, assessed learner synthesis and `MORE_EVIDENCE_NEEDED`.

Day91 completion record:

- [x] Isolate SDK Server types in the Adapter and convert inbound requests to application-owned DTOs.
- [x] Add bounded Tool, Resource and Prompt handlers with narrow dependency-injected services.
- [x] Add signed revision-bound Tool pagination, pre-handler backpressure and bounded drain/reconciliation.
- [x] Add 40 focused tests, 14 seed cases, deterministic example, design, classroom record and Day92 handoff.
- [x] Run 621 cumulative tests and available Day83–Day91 seed suites on Python 3.11.5.
- [x] Record `INTEGRATION_RUNTIME`, assessed learner synthesis and `MORE_EVIDENCE_NEEDED`.
...
- [x] Verify official MCP specification/schema `2026-07-28` and keep old initialize/session semantics historical.
- [x] Add SDK-independent DTOs, local request/operation binding, per-request capability validation and Fake Transport.
- [x] Add correlation, observation-only results, Resource/Prompt checks and reconciliation evidence.
- [x] Add 21 focused/grader tests, 16 seed cases, example, design, classroom record and Day90 handoff.
- [x] Run 560 cumulative tests, Day88 regression and Day83–Day89 seed suites on Python 3.11.5.
- [x] Preserve independent learner synthesis as assessed with corrections and real MCP/production boundaries as NOT RUN.
...
- [x] Release the 16-section lesson without pre-locking or installing a framework.
- [x] Add bounded official framework evidence and 12 active, deduplicated primary-source job postings from
  9 companies, with version conflicts and sample biases preserved.
- [x] Add the offline validator, 22 tests and deterministic `MORE_EVIDENCE_NEEDED` Day88 handoff.
