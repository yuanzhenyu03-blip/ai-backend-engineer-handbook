...
- [ ] Day68 prep: define the FastAPI <-> n8n polling/callback contract (status endpoint + callback shape) so a workflow can observe long-running AI Job progress without direct DB access.
- [ ] Day68 prep: specify correlation-ID propagation (request_id/task_id/trace_id) across the n8n execution and the FastAPI Job lifecycle.
- [ ] Day68 prep: specify business idempotency for redelivery/retry so a re-sent request or a callback returns the existing task_id and never duplicates work (reuse Day59/Day50 acceptance + the Day67 `request_id` idempotency record).
- [ ] Day68 prep: define a scheduled reconciliation idempotency basis (`task_id + recovery_action + recovery_generation`) callable via an authenticated FastAPI reconciliation API (never a direct `browser_tasks` scan).
