...
(Semantics, shared with PROJECT_STATUS.md: "Current Lesson" = the lesson currently being worked on / next up (Day69);
Day68 is finished and most recently released; Day59–67 are complete. The Phase 5 Production Integration Gate (Day59–61) is COMPLETE.)
...
- [ ] Day69 prep: design the human-approval gate on top of Day68's stable `task_id`/`correlation_id`/`event_id`, so approval is a durable backend decision, not an n8n-local state.
- [ ] Day69 prep: specify error/retry workflows that reuse Day68 idempotency (reused `request_id`, dedupe `event_id`) and never blindly recreate paid work.
- [ ] Day69 prep: define Secrets handling and callback authentication for the callback boundary named in Day68 (no secrets in n8n execution data; short-lived scoped delegation).
- [ ] Day69 prep: define the audit contract (correlation_id-linked, backend-authoritative) distinguishing orchestration evidence from durable business truth.
