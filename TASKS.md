...
- [x] Complete Day70 n8n + FastAPI + AI Tool Integration Capstone and Interview (Phase 6 close) — lesson (16-section v2, classroom-loop Concepts + exercises + full English Interview; preserved the real short student answers/corrections and the Tech-Lead-taught final synthesis), the capstone artifacts under `projects/n8n-workflows/` (DAY70_CAPSTONE.md + day70_capstone.py + test_day70_capstone.py (18 tests; classroom 14 pre-fix superseded) + Secret-free day70_minimal_acceptance_workflow.json with two independent auth boundaries and a `{document_ids, business_input:{report_scope}}` body), FastAPI/n8n interview + cheat-sheet appends, and CURRICULUM/ROADMAP/PROJECT_STATUS/CHANGELOG updates (Day70 Completed; Phase 6 COMPLETE; Day71 Planned Phase 7A phase-transition). Mixed evidence tiers: a bounded authenticated n8n->FastAPI SERVICE-call acceptance slice INTEGRATION_RUNTIME (in class, PRE-FIX workflow; FIXED workflow NOT RERUN; inbound caller->n8n auth NOT RUN), decision model + workflow static contract EXECUTED_LOCAL_RUNTIME (18 passed; Day59 fingerprint test 12 passed), rest CONCEPTUAL_STATIC / NOT RUN; no post-run n8n export captured; no secrets committed.
