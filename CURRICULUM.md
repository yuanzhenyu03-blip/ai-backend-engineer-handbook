...
- Day70 — n8n + FastAPI + AI Tool Integration Capstone and Interview.
  ...
  Status: ✅ Completed. Released Lesson: `docs/fastapi/day70-n8n-fastapi-ai-tool-integration-capstone-and-interview.md`. Engineering Artifacts under `projects/n8n-workflows/`: `DAY70_CAPSTONE.md`, `day70_capstone.py` (standalone decision model), `test_day70_capstone.py` (28 tests; the classroom's 14 pre-fix tests are superseded), and `day70_minimal_acceptance_workflow.json` (Secret-free importable Workflow SOURCE with two independent auth boundaries — inbound Webhook `headerAuth` and outbound HTTP `httpHeaderAuth`, both Credential Store references only; the request body is `{document_ids, business_input:{report_scope}}` so report_scope enters the Day59 fingerprint; NOT a captured post-run export). Cumulative path: authenticated-service trigger (inbound caller->n8n auth NOT RUN) -> map/validate report_scope+request_id+document_id -> FastAPI durable acceptance -> honest 202 + stable Task id -> observe same Task -> permissioned execution -> verified protected Artifact -> exact-version Approval -> idempotent Publication -> correlated audit. n8n orchestrates; FastAPI/PostgreSQL owns durable truth/authz/idempotency/transitions/recovery/audit (Day67–Day69 boundary unchanged). Evidence (four-tier taxonomy): EXECUTED_LOCAL_RUNTIME — `day70_capstone.py` + the workflow static contract via `test_day70_capstone.py` (classroom pre-fix suite Python 3.11.5 = 14 passed, now SUPERSEDED; a first attempt on Python 3.9.6 reported pytest missing = NOT RUN, not a failure; the updating agent re-ran the FIXED suite on Python 3.10.12 = 21 passed and the affected Day59 fingerprint test = 12 passed; repository-standard Python 3.12 = NOT RUN) plus a real n8n inspection alone (n8n 2.25.6, /healthz 200). INTEGRATION_RUNTIME (performed in class against disposable local infra; NOT re-run by the updating agent) for the PRE-FIX workflow (top-level report_scope; no inbound Webhook auth) — a bounded real, authenticated n8n Workflow -> FastAPI/Uvicorn -> PostgreSQL SERVICE-call ACCEPTANCE slice: POST /webhook/day70/research-report -> 202 + stable Task id; exact redelivery -> same id + idempotency_replayed=true; new-connection DB jobs=1/outbox=1/document_links=1 + 64-char fingerprint + queued; invalid input -> 400 with zero Jobs. This proves the acceptance boundary ONLY; it did NOT prove inbound caller->n8n auth and is NOT evidence for the FIXED workflow (report_scope in business_input, three-field IF, inbound headerAuth reference), whose real integration is NOT RERUN and whose different-report_scope 409 path is proven only by the Day59 fingerprint contract test. NOT RUN: budget reservation in the acceptance transaction; a FastAPI-persisted/returned correlation_id and its propagation; real Polling/Callback/Approval/Publication/Error-Workflow runtime; real Worker/Outbox-Relay/broker/Browser-Tool/Provider execution; verified Artifact generation/lookup; real Approval schema/migration/UI/callback/tenant-role authorization + atomic audit; real Publication target/operation idempotency/external reconciliation; real credential revoke/rotation/exposure review (only controlled local Credential Store injection ran); inbound Webhook authentication (external caller -> n8n) — a source reference only, NOT RUN / NOT CONFIGURED at run time; a real re-run of the FIXED workflow end-to-end (NOT RERUN); rollback/kill-switch/canary incident exercise; Python 3.12; a captured n8n post-run export; production. No real or paid Provider call; the final Chinese synthesis was taught directly by the Tech Lead after the student requested it. A green test suite or a single integration run is not cumulative integration and is not production; Day59–Day69 evidence is a named prerequisite, not Day70 validation. No secrets, tokens, real URLs, tenant data, or Provider payloads committed. Day70 closes Phase 6.
...
- Day72 — Provider Capabilities and the Replaceable Provider Adapter.
  ...
  Status: ✅ Completed (classroom scope). Released Lesson: `docs/fastapi/day72-provider-capabilities-and-the-replaceable-provider-adapter.md`. Engineering Artifact: `projects/ai-agent/` (Phase 7A — `src/provider_contract.py` + `src/provider_adapters.py` + `tests/test_provider_adapters.py` + `docs/DAY72_PROVIDER_ADAPTER.md` released design contract + `docs/day72-provider-adapter-classroom-draft.md`; README updated). DIRECT PREREQUISITES: Day71 (provider-independent runtime), Day53 (ProviderRequest -> ProviderOutcome seam / SDK-type containment / untrusted success), Day61 (real-HTTP Adapter foundation: correlation vs Provider request id; timeout != non-execution). Core model: the application owns a STABLE product contract (research_claims.v1 + required citations); Providers offer only VERSIONED capabilities bound to a Capability Profile (profile_id + provider + model + api_version + profile_version + adapter_version + supported_contracts + requires_request_identity + verification_tier + status). Capability admission runs BEFORE a paid call (incompatible -> CAPABILITY_ERROR + 0 calls; never weakened to a lowest-common-denominator; a passing pre-call check does not guarantee runtime success). Published profile revisions are immutable audit facts; drift disables/quarantines the old revision for NEW selections and a new revision is published (never edit v3 in place); evidence ladder DECLARED -> STATIC -> EXECUTED_LOCAL_RUNTIME -> INTEGRATION_RUNTIME -> PRODUCTION. The Adapter does bidirectional translation by SEMANTIC equivalence (Provider A finish_reason=length and Provider B completionState=MAX_TOKENS both -> TRUNCATION), keeps SDK/wire types inside, and preserves a stable failure taxonomy (SUCCESS | PROVIDER_RESPONSE_INVALID | REFUSAL | TRUNCATION | RATE_LIMITED | AUTHENTICATION_ERROR | CAPABILITY_ERROR | TIMEOUT_UNKNOWN | TRANSPORT_ERROR); it observes/classifies facts + safe evidence and does NOT hide retries, switch Providers, or decide Job terminal state (a new call = a new Attempt). Provider wire/envelope validation = Adapter (missing required request id -> PROVIDER_RESPONSE_INVALID); application schema/evidence/policy validation = Runtime (nonexistent citation -> evidence gate). Each Attempt persists its execution contract (current config governs new calls; the bound version interprets an already-issued call; never rewritten). Replaceability = equivalent application result/failure semantics, not identical bytes; business code depends only on the `ProviderAdapter` Protocol with a `ProviderRegistry`/composition boundary that fails closed on DISABLED/QUARANTINED. Client ProductOption is a selector; server allowlist policy is the authority. If a Profile is disabled before dispatch, the planned Attempt is retained as BLOCKED_PROFILE_DISABLED (provider_calls=0) and never rewritten. Provider SUCCESS is an untrusted candidate (schema -> evidence -> policy -> guarded completion); TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION (most dangerous class, no bulk A2). Rollback of a bad capability-profile-v4 stops future harm only, scopes the affected set by profile/release version + time window + durable evidence, and classifies per Job without bulk retry/success, Attempt overwrite, or history deletion. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 58 deterministic in-process tests OK, Python 3.12.13; RecordingTransport + fixtures only). NOT RUN: INTEGRATION_RUNTIME (no real SDK/HTTP/Provider/PostgreSQL/external process), PRODUCTION (no credentials/customer data/paid call/production traffic). Provider A/B are fictional classroom fixtures; capabilities are versioned facts, not permanent claims; the full Fake Provider/LLM regression suite remains Day77 scope. The final Chinese Mental Model synthesis was supplied by the Tech Lead at the student's explicit request. Next: Day73 makes the prompt a versioned contract on this stable Provider surface.
- Day73 — Prompt Contracts, Prompt Versioning and Compatibility.
  ...
  Status: ✅ Completed (classroom scope). Released Lesson: `docs/fastapi/day73-prompt-contracts-prompt-versioning-and-compatibility.md`. Engineering Artifact: `projects/ai-agent/` (Phase 7A — `src/prompt_contracts.py` + `tests/test_prompt_contracts.py` + `docs/DAY73_PROMPT_CONTRACTS.md` released design contract + `docs/day73-prompt-contracts-classroom-draft.md`; README updated). DIRECT PREREQUISITES: Day72 (replaceable Provider Adapter + versioned capability admission), Day71 (provider-independent runtime), Day53 (Provider boundary / untrusted success), Day54 (Attempt lifecycle / timeout-unknown / late-result identity binding). Core model: a Prompt Contract is an APPLICATION-OWNED, versioned execution contract (not a mutable string, not a Provider capability declaration) with a stable `prompt_contract_id`, an immutable `revision`, ordered message templates + roles, required/optional variables + input schema, semantic guarantees (e.g. `citations_required`), compatible application contracts, and a renderer version; operational status is a SEPARATE lifecycle overlay (ACTIVE/DISABLED/QUARANTINED; an unregistered revision is UNKNOWN and every caller fails closed). Each Attempt durably binds the revision + renderer version + independently-versioned model-parameter-policy id/revision + application contract + canonical input fingerprint + rendered-message hash (+ optional protected rendered-artifact reference). The CURRENT DEFAULT selects a revision for a NEW Attempt only; it never reinterprets an already planned/dispatched Attempt. Responsibility boundaries kept distinct: application business authorization (a prompt variable is never authorization evidence), LLM Runtime (selection/binding/lifecycle/variable+schema validation/compatibility/deterministic rendering/binding verification/guarded transitions/reconciliation), Model Parameter Policy (temperature/limits, versioned independently), Provider Adapter (provider translation only — never chooses/rewrites the revision), and output/business validation (a transport SUCCESS is an untrusted candidate). The LLM Runtime enforces a pre-Provider gate BEFORE Day72 capability admission: a dispatch-vs-binding mismatch is rejected before a valid transition (Attempt stays PLANNED, provider_calls=0, BINDING_MISMATCH); a disabled/quarantined BOUND revision is a guarded PLANNED -> BLOCKED_PROMPT_DISABLED (provider_calls=0); unknown revision / incompatible contract / invalid variables / input-fingerprint mismatch / rendered-hash mismatch each fail closed with zero calls; only a fully-verified Attempt proceeds (READY). Compatibility is DIRECTIONAL and both STRUCTURAL and SEMANTIC: an optional variable with a safe default is backward compatible; a rename without an alias/migration is a structural break; weakening a guarantee (citations required -> optional) is a semantic break even if schema is unchanged; an unknown policy value fails closed; a conflicting alias+replacement is rejected; migration is explicit, non-mutating, auditable, and never rewrites a historical Attempt. Audit logs keep only minimal evidence (ids/revisions/fingerprint/rendered-hash/decision); a full rendered prompt is a protected artifact (encryption, tenant-aware authz, access audit, finite retention); a hash is integrity evidence not encryption, and a `rendered_artifact_ref` is a pointer not authorization. Incident verbs are distinct: disable / rollback / migration / re-release; a bad `v2` is disabled, the new-planning default rolls back to `v1`, Attempts are classified per persisted evidence (PLANNED-never-dispatched -> block/re-plan; dispatched-successful -> untrusted candidate + revalidate guarantee; definitely-invalid -> explicit failure; TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION, no blind retry; valid-late -> interpret with the BOUND revision; stale/terminal/superseded -> never overwrite), and a corrected `v3` is published immutably under controlled rollout. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 97 deterministic in-process tests OK: 39 Day73 + 58 Day72 regression, Python 3.11.5; in-memory stores + pure functions only). NOT RUN: INTEGRATION_RUNTIME (no real SDK/HTTP/Provider/PostgreSQL/queue/worker/encryption/protected-artifact storage), PRODUCTION (no credentials/customer data/sensitive prompt/production traffic). The `provider_calls` counter models an in-process boundary crossing only. Day72's existing tests are prerequisite evidence, not Day73 validation; the full Fake Provider/LLM regression suite remains Day77 scope. The final Chinese Mental Model synthesis was supplied by the Tech Lead at the student's explicit request. Next: Day74 constrains the model output with structured output, JSON Schema and function/tool calling on this versioned input boundary.
- Day74 — Structured Output, JSON Schema and Function/Tool Calling.
  ...
  Status: ✅ Completed (classroom scope). Released Lesson: `docs/fastapi/day74-structured-output-json-schema-and-function-tool-calling.md`. Engineering Artifact: `projects/ai-agent/` (`src/output_tool_contracts.py` + `tests/test_output_tool_contracts.py` + `docs/DAY74_OUTPUT_TOOL_CONTRACTS.md` + `docs/day74-output-tool-contracts-classroom-draft.md`; README updated). DIRECT PREREQUISITES: Day73 (versioned Prompt/Attempt input binding), Day72 (Provider Adapter/capability facts), Day53 (untrusted Provider success), Day54 (Attempt/timeout/late-result identity), and Day44 contract thinking. Core model: Provider response/tool call is an untrusted candidate; Parse -> strict Schema -> exact Registry name/version/lifecycle -> trusted-context Authorization -> tenant-scoped Semantics -> per-candidate Tool Admission -> immutable server-normalized `AdmittedToolCall` -> final disable guard + atomic idempotency claim -> Tool Execution -> Outcome Schema/Semantics/Identity Verification -> guarded durable completion/reconciliation. Model `tenant_id` is never authority; unknown fields/tools/versions and disabled versions fail closed; Schema success is not semantics or permission; tool success is not durable Job success. `TIMEOUT_UNKNOWN` -> `PENDING_RECONCILIATION`, no blind retry; valid late results require exact current identity; stale/terminal/superseded results have zero effect. Incident actions remain distinct: reject/re-prompt/disable/rollback/repair/compensation/reconciliation, never rewrite historical bindings or delete evidence. Evidence: CONCEPTUAL + STATIC (`py_compile` + `mypy`) + EXECUTED_LOCAL_RUNTIME (`python3.11 -m unittest discover -s tests -v` -> 131 tests OK: 34 Day74 + 39 Day73 + 58 Day72, Python 3.11.5). Python 3.12, full JSON Schema compliance, INTEGRATION_RUNTIME and PRODUCTION are NOT RUN; no real Provider/SDK/HTTP/database/queue/external tool/reconciliation. The schema validator is an explicit teaching subset; locks/counters are in-process models only. Final Chinese synthesis was teaching-assistant-supplied at the student's explicit request. Next: Day75 must preserve complete validated output, identity, authorization and completion boundaries while adding streaming/caching/batching.
