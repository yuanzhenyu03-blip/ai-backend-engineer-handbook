...
Day: Day72 (Phase 7A) third-round review fixes. Local, unreleased hardening of the replaceable Provider Adapter slice. No new lesson day and no new released version number; the released Day72 lesson, design contract and status remain Day72.

Changed:

- `projects/ai-agent/src/provider_contract.py` — the lifecycle authority is now FAIL-CLOSED and explicit at the trusted composition boundary. Added `ProfileStatus.UNKNOWN`; `ProfileLifecycle.status()` returns `UNKNOWN` (never `ACTIVE`) for a profile the authority never registered; added `UnknownProfileError`. `ProviderRegistry` and `ProviderSelectionPolicy` both REQUIRE the shared lifecycle authority—neither may silently fabricate or fall back to a fresh ACTIVE catalog. `ProviderRegistry.get_selectable()` and `resolve_bound_attempt()` fail closed on an unknown profile (`UnknownProfileError`); selection distinguishes UNKNOWN (`UnknownProfileError`) from DISABLED/QUARANTINED (`ProfileDisabledError`).
- `projects/ai-agent/src/provider_adapters.py` — `dispatch_attempt` now takes `(request, contract, state_store, registry)` and no longer accepts a caller-supplied Adapter or lifecycle: it reads the authoritative contract from the store, resolves the bound Adapter through the same authoritative Registry, and reads the current status from that Registry's OWN lifecycle catalog (an UNKNOWN status raises `UnknownProfileError` with zero calls). Added a structured `ExecutionCertainty` (`DEFINITELY_NOT_SENT` / `EXECUTION_UNKNOWN`) carried on each Provider SDK base exception, plus a last-line `except ProviderASDKError` / `except ProviderBSDKError` base catch that maps an unknown SDK error by certainty (`DEFINITELY_NOT_SENT -> TRANSPORT_ERROR`; `EXECUTION_UNKNOWN`/missing -> `TIMEOUT_UNKNOWN`, conservative) with no message-string classification and no SDK object/message in the outcome; a non-SDK programming error still propagates. Added strict allowlist normalizers `normalize_retry_after` (ASCII decimal seconds, `0..86400`, canonicalized; drops whitespace/sign/decimal/control/HTTP-text/overlong/secret-like) and `normalize_request_id` (short ASCII allowlist), applied to the rate-limit `retry_after` and to `provider_request_id` in translation and timeout handlers; an invalid value is dropped entirely and never appears in `detail`.
- `projects/ai-agent/tests/test_provider_adapters.py` — rewritten for the new registry-based dispatch API and expanded to 58 deterministic in-process tests: a dispatch-signature assertion that the params are exactly `{request, contract, state_store, registry}` and NOT `{adapter, lifecycle, transport}`; fail-closed lifecycle lookups; both Registry and SelectionPolicy require the shared lifecycle; a replacement Registry wired to the same authority cannot reactivate a disabled v3 or make a call; active+compatible still calls once; register+disable(+quarantine) via the Registry reach `BLOCKED_PROFILE_DISABLED` with zero calls and A1 still v3 / no A2; an unknown lifecycle status at dispatch fails closed with zero calls; forged caller-contract and forged-drifted-Registry rejection with zero calls; the two-thread `threading.Barrier` one-call concurrency test (repeated 40x); unknown-SDK containment (`DEFINITELY_NOT_SENT -> TRANSPORT_ERROR`, `EXECUTION_UNKNOWN`/missing -> `TIMEOUT_UNKNOWN`, no message/secret leak, one attempt, no propagation, non-SDK error not swallowed); and `retry_after`/`request_id` normalization (valid canonicalization + invalid values dropped, never in `detail`/`safe_evidence`).

Docs synced (current-state surfaces; historical CHANGELOG entries left intact):

- `projects/ai-agent/docs/DAY72_PROVIDER_ADAPTER.md` — added a round-3 hardening section (§8b) and updated the EXECUTED_LOCAL_RUNTIME test count 28 -> 58.
- `projects/ai-agent/README.md`, the Day72 lesson, `projects/ai-agent/docs/day72-provider-adapter-classroom-draft.md`, `cheat_sheets/fastapi.md`, `interview/fastapi.md`, `CURRICULUM.md`, `ROADMAP.md`, `PROJECT_STATUS.md`, `TASKS.md` — Day72 EXECUTED_LOCAL_RUNTIME test count 28 -> 58 (current-state surfaces only; Day58/Day63/Day70 counts untouched). AGENTS.md already reflects Day72 Completed / Day73 current.

Evidence (four-tier taxonomy): CONCEPTUAL — completed. STATIC — `PASS` (`python3 -m py_compile` on both modules + the test module; `mypy` on all three files; required doc sections; balanced fences; whitespace/credential scans). EXECUTED_LOCAL_RUNTIME — `PASS`: `python3 -m unittest discover -s tests -v` -> 58 deterministic in-process tests OK (Python 3.12.13), `RecordingTransport` + fixtures only; the concurrency test is DETERMINISTIC IN-PROCESS evidence only and the in-memory Attempt state store MODELS compare-and-set (RLock) — it is NOT a durable database (production needs a DB conditional UPDATE / transaction). INTEGRATION_RUNTIME — NOT RUN (no real SDK, HTTP process boundary, Provider, PostgreSQL, or external process). PRODUCTION — NOT RUN (no credentials, customer data, paid call, or production traffic). Provider A/B wire fields, SDK error types and execution-certainty values are fictional classroom fixtures; capabilities are versioned facts; the full Fake Provider/LLM regression suite remains Day77 scope. No stable product contract was weakened. No real/paid Provider call, network, or credential was introduced.

---

## v0.1.172 — Day72 review fixes (round 2): authoritative Attempt binding + thread-safe CAS + two-purpose Registry + Adapter failure translation (Phase 7A)

Date: 2026-08-21

Day: Day72 (Phase 7A) second-round review fixes. Hardens the replaceable Provider Adapter slice further. No new lesson day; the released Day72 lesson, design contract and status remain Day72.

Changed:

- `projects/ai-agent/src/provider_contract.py` — the Attempt state store now holds the AUTHORITATIVE immutable `AttemptExecutionContract` (a new immutable `AttemptRecord` of contract + state); `plan(contract)` registers it and rejects a duplicate `attempt_id`; `get_record(attempt_id)` returns it; `compare_and_set(contract, expected, new)` is bound to identity + full binding + expected state and runs under a `threading.RLock` (atomic check+write for in-process threads). Added a `ProfileLifecycle` overlay (current operational status, separate from the immutable published `CapabilityProfile` revisions); removed the mutable state field usage from the contract. `ProviderRegistry` now offers two purposes — `get_selectable(profile_id, application_contract)` (new tasks: ACTIVE + supports-contract, fail closed, no fallback) and `resolve_bound_attempt(contract)` (already-persisted Attempt: resolve the bound Adapter even if the profile is now disabled/quarantined, verifying its immutable binding) — plus `disable()`/`quarantine()` lifecycle updates and duplicate-`profile_id` rejection. `ProviderSelectionPolicy` can consult the shared lifecycle overlay. `admit_capability` now takes the current lifecycle status.
- `projects/ai-agent/src/provider_adapters.py` — added fictional, structured Provider-specific SDK exception TYPES for Provider A and Provider B; `RecordingTransport` can return a wire response OR raise a scripted SDK error (recording the send attempt first); each Adapter's `execute()` translates its own Provider's failures into the stable surface (REFUSAL / RATE_LIMITED / AUTHENTICATION_ERROR / TIMEOUT_UNKNOWN / TRANSPORT_ERROR) and never raises an SDK exception, never retries, never switches Provider, never creates the next Attempt, and never downgrades `TIMEOUT_UNKNOWN` to a retryable `TRANSPORT_ERROR`. `dispatch_attempt` now reads the authoritative contract from the store, rejects a caller-supplied contract that differs from it (`AttemptBindingError`, zero calls, state unchanged), admits against the CURRENT lifecycle status (guarded `PLANNED -> BLOCKED_PROFILE_DISABLED` for a disabled profile), and guards the single external call with the thread-safe CAS; it takes a `lifecycle` argument (still no `transport`). `provider_calls` is 0 before a send attempt and 1 once a send attempt begins.
- `projects/ai-agent/tests/test_provider_adapters.py` — expanded from 21 to 28 tests, adding: authoritative-binding rejection of a self-consistent forged request+contract+Adapter trio (zero calls, store unchanged) + duplicate-plan rejection + read-back identity + CAS bound to identity/binding/state; a real two-thread `threading.Barrier` concurrency test (repeated 40x) proving exactly one external call, exactly one `AttemptStateError`, transport calls == 1, final state DISPATCHED; the end-to-end Registry path (register A/v3 -> plan A1(v3) -> `disable()` via lifecycle -> `get_selectable` rejects -> `resolve_bound_attempt` resolves -> dispatch reaches BLOCKED_PROFILE_DISABLED, 0 calls, A1 still v3, no A2) with no directly-constructed disabled Adapter; and per-Provider failure-translation parity for REFUSAL/RATE_LIMITED/AUTHENTICATION_ERROR/TIMEOUT_UNKNOWN/TRANSPORT_ERROR/PROVIDER_RESPONSE_INVALID/TRUNCATION/SUCCESS, including a timeout-through-dispatch test proving no second call / no auto A2 and a no-leak assertion.

Docs synced (current-state surfaces; historical CHANGELOG entries left intact):

- `projects/ai-agent/docs/DAY72_PROVIDER_ADAPTER.md` — added a round-2 hardening section (authoritative binding, thread-safe CAS + in-process-only caveat, two-purpose Registry + lifecycle overlay, concrete failure translation) and updated the Files block; test count 21 -> 28.
- `projects/ai-agent/docs/day72-provider-adapter-classroom-draft.md`, the Day72 lesson, `cheat_sheets/fastapi.md`, `interview/fastapi.md`, `CURRICULUM.md`, `ROADMAP.md`, `PROJECT_STATUS.md`, `TASKS.md`, `projects/ai-agent/README.md` — test count 21 -> 28 and the round-2 invariants noted. AGENTS.md already reflected Day72 Completed / Day73 current.

Evidence (four-tier taxonomy): CONCEPTUAL — completed. STATIC — `PASS` (`python3 -m py_compile` on both modules; required doc sections; balanced fences; whitespace/credential scans). EXECUTED_LOCAL_RUNTIME — `PASS`: `python3 -m unittest discover -s tests -v` -> 28 deterministic in-process tests OK (Python 3.10.12), `RecordingTransport` + fixtures only; the concurrency test is DETERMINISTIC IN-PROCESS evidence only and the in-memory Attempt state store MODELS compare-and-set (RLock) — it is NOT a durable database (production needs a DB conditional UPDATE / transaction). INTEGRATION_RUNTIME — NOT RUN (no real SDK, HTTP process boundary, Provider, PostgreSQL, or external process). PRODUCTION — NOT RUN (no credentials, customer data, paid call, or production traffic). Provider A/B wire fields and SDK error types are fictional classroom fixtures; capabilities are versioned facts; the full Fake Provider/LLM regression suite remains Day77 scope. No stable product contract was weakened. No real/paid Provider call, network, or credential was introduced.

---

## v0.1.171 — Day72 review fixes: enforced Attempt binding + single-call guard + adapter/selection/registry hardening (Phase 7A)

Date: 2026-08-21

Day: Day72 (Phase 7A) review fixes. Hardens the replaceable Provider Adapter slice per the Day72 code review. No new lesson day; the released Day72 lesson, design contract and status remain Day72.

Changed:

- `projects/ai-agent/src/provider_contract.py` — added `AttemptBindingError` + `validate_attempt_binding()` (job_id/application_contract/profile_id/provider_name/model/api_version/profile_version/adapter_version must match exactly before any call — an application invariant, never a Provider failure); made `AttemptExecutionContract` a pure immutable binding (removed the mutable `state` field); added an `AttemptStateStore` Protocol + `InMemoryAttemptStateStore` with compare-and-set semantics (PLANNED/DISPATCHED/BLOCKED_PROFILE_DISABLED); added `AttemptStateError`, `DuplicateProfileRegistrationError`, `ProviderIncompatibleError`; `ProviderRegistry.register()` now rejects a conflicting duplicate `profile_id` (idempotent same-instance re-register allowed); `ProviderSelectionPolicy.select()` now REQUIRES the Profile to support the requested application contract (no lowest-common-denominator downgrade, no automatic fallback); added `execute()` to the `ProviderAdapter` Protocol.
- `projects/ai-agent/src/provider_adapters.py` — each concrete Adapter now owns its constructor-injected transport and exposes `execute()`; removed the second-transport injection from `dispatch_attempt` (it takes `request, contract, adapter, state_store` — no `transport`). `dispatch_attempt` enforces the pre-call gate order — binding validation -> capability admission (disabled -> guarded PLANNED -> BLOCKED_PROFILE_DISABLED) -> compare-and-set PLANNED -> DISPATCHED — then makes AT MOST ONE external call; `provider_calls` reflects THIS dispatch (0 or 1), never the transport's cumulative history; a repeated or non-PLANNED dispatch is rejected with zero new calls.
- `projects/ai-agent/tests/test_provider_adapters.py` — expanded from 7 to 21 deterministic in-process tests covering: binding mismatches (wrong Provider adapter / application_contract / job_id / profile_version / adapter_version -> AttemptBindingError, zero calls); one-call-per-Attempt (first=1 call+DISPATCHED, second=rejected+0 new, already-DISPATCHED not called, transition-failure=0); transport-inside-adapter (no `transport` dispatch parameter; Registry-injected adapter dispatches without business passing a transport; distinct A/B wire but equivalent SUCCESS); selection-by-contract (compatible selected, incompatible -> ProviderIncompatibleError with no fallback, disabled -> ProfileDisabledError); duplicate registry rejection + idempotent same-instance; the real disabled-profile -> BLOCKED_PROFILE_DISABLED dispatch path (A1 keeps v3, A2 is a new Attempt); and preserved semantics (pre-call CAPABILITY_ERROR, two output-limit states -> TRUNCATION, missing request id -> PROVIDER_RESPONSE_INVALID).

Docs synced:

- `AGENTS.md` — corrected the stale progress block (was "Day69 last / Day70 not started"): Last Completed = Day72; Current/Next = Day73; Current phase = Phase 7A (Day71–Day72 Completed, Day73 current); Phase 6 COMPLETE. No Day73 content created.
- `projects/ai-agent/docs/day72-provider-adapter-classroom-draft.md` — no longer claims "not GitHub truth / not released"; described as a released classroom-process record; Validation Status corrected (CONCEPTUAL Completed; EXECUTED_LOCAL_RUNTIME PASS at 21 tests); INTEGRATION_RUNTIME/PRODUCTION remain NOT RUN.
- `projects/ai-agent/docs/DAY72_PROVIDER_ADAPTER.md`, the Day72 lesson, `cheat_sheets/fastapi.md`, `interview/fastapi.md`, `CURRICULUM.md`, `ROADMAP.md`, `PROJECT_STATUS.md`, `TASKS.md`, `projects/ai-agent/README.md` — test count corrected 7 -> 21 and the enforced invariants documented (binding validation, single-call state store, transport-inside-adapter, selection-by-contract, duplicate-registry rejection, real disable path).

Evidence (four-tier taxonomy): CONCEPTUAL — completed. STATIC — `PASS` (`python3 -m py_compile` on both modules; required doc sections; balanced fences; whitespace/credential scans). EXECUTED_LOCAL_RUNTIME — `PASS`: `python3 -m unittest discover -s tests -v` -> 21 deterministic in-process tests OK (Python 3.10.12), `RecordingTransport` + fixtures only; the in-memory Attempt state store MODELS compare-and-set and is NOT a durable database. INTEGRATION_RUNTIME — NOT RUN (no real SDK, HTTP process boundary, Provider, PostgreSQL, or external process). PRODUCTION — NOT RUN (no credentials, customer data, paid call, or production traffic). Provider A/B wire fields are fictional classroom fixtures; capabilities are versioned facts, not permanent claims; the full Fake Provider/LLM regression suite remains Day77 scope. No stable product contract was weakened. No real/paid Provider call, network, or credential was introduced.
...
- `projects/ai-agent/tests/test_provider_adapters.py` — 21 deterministic in-process tests (the original 7-test slice plus the Day72 review regressions).
...
- `ROADMAP.md` — Day72 table row Planned -> ✅ Completed (CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME 21 tests; INTEGRATION/PRODUCTION NOT RUN).
...
Day: Day70 (Phase 6, capstone). Integrates the Day67–Day69 orchestration contracts into one failure-aware cumulative path and runs the maximum feasible real `n8n -> FastAPI -> PostgreSQL` acceptance slice, with strict runtime-evidence boundaries. n8n gains no authority over durable facts.

Added:

- `docs/fastapi/day70-n8n-fastapi-ai-tool-integration-capstone-and-interview.md` — the 16-section `LESSON_TEMPLATE_v2` lesson (classroom-loop Concepts, exercises, full English Interview), preserving the real short student answers/corrections and the Tech-Lead-taught final Chinese synthesis.
- `projects/n8n-workflows/DAY70_CAPSTONE.md` — cumulative path, responsibility boundary, evidence matrix, commands/results, rollback exercise, and final Mental Model.
- `projects/n8n-workflows/day70_capstone.py` — a standalone deterministic decision model (acceptance recovery, event dedupe/conflict, exact Approval binding, Publication recovery, credential classification, incident Task classification, polling boundary).
- `projects/n8n-workflows/test_day70_capstone.py` — 21 deterministic tests (the classroom's 14 pre-fix tests are superseded).
- `projects/n8n-workflows/day70_minimal_acceptance_workflow.json` — a Secret-free importable Workflow source (`Day70 - FastAPI Durable Acceptance Gate`; two independent auth boundaries — inbound Webhook `headerAuth` and outbound HTTP `httpHeaderAuth`, both Credential Store references only; request body `{document_ids, business_input:{report_scope}}` so report_scope enters the Day59 fingerprint; NOT a captured post-run export).

Updated: `projects/n8n-workflows/README.md` (Day70 cumulative path + evidence matrix + NOT RUN backlog + links), `cheat_sheets/fastapi.md`, `interview/fastapi.md`, `interview/n8n.md` (Phase 6 section), `CURRICULUM.md`, `ROADMAP.md`, `PROJECT_STATUS.md`, `TASKS.md` (Day70 Completed; Phase 6 COMPLETE; Day71 Planned Phase 7A as a phase transition, not an n8n dependency).

Evidence (four-tier taxonomy): EXECUTED_LOCAL_RUNTIME — `day70_capstone.py` + the workflow static contract via `test_day70_capstone.py` (classroom pre-fix suite Python 3.11.5 = 14 passed, SUPERSEDED; a first attempt on Python 3.9.6 reported pytest missing = NOT RUN, not a failure; the updating agent re-ran the FIXED suite on Python 3.10.12 = 21 passed and the affected Day59 fingerprint test = 12 passed; repository-standard Python 3.12 = NOT RUN) + a real n8n inspection alone. INTEGRATION_RUNTIME (performed in class against disposable local infra; NOT re-run by the updating agent) for the PRE-FIX workflow (top-level report_scope; no inbound Webhook auth) — a bounded real, authenticated n8n Workflow -> FastAPI/Uvicorn -> PostgreSQL SERVICE-call ACCEPTANCE slice (202 + stable Task id; exact redelivery -> same id + idempotency_replayed=true; new-connection DB jobs=1/outbox=1/document_links=1 + 64-char fingerprint + queued; invalid input -> 400 with zero Jobs) — acceptance boundary only; it did NOT prove inbound caller->n8n auth and is NOT evidence for the FIXED workflow (NOT RERUN; the different-report_scope 409 is proven only by the Day59 fingerprint contract test). NOT RUN: inbound Webhook authentication (external caller->n8n; source reference only, NOT CONFIGURED at run time); budget reservation; correlation_id persist/propagate; real Polling/Callback/Approval/Publication/Error-Workflow runtime; real Worker/Outbox-Relay/broker/Browser-Tool/Provider execution; verified Artifact; real Approval schema/migration/UI/tenant-role authz + atomic audit; real Publication target/operation idempotency/external reconciliation; real credential revoke/rotation/exposure review; rollback/kill-switch/canary; Python 3.12; a captured n8n post-run export; production. A green test suite or a single integration run is not cumulative integration and is not production; Day59–Day69 evidence is a named prerequisite, not Day70 validation. No real/paid Provider call, production credentials, customer data, secrets, tokens, real URLs, or tenant fixtures are committed.
