...
Engineering Artifact: The Day48 Alembic control plane ([`projects/ai-backend-data-layer/api/day48-alembic-safe-schema-evolution-design.md`](../../projects/ai-backend-data-layer/api/day48-alembic-safe-schema-evolution-design.md)) with a runnable Alembic package [`day48_alembic/`](../../projects/ai-backend-data-layer/api/day48_alembic) (minimal `env.py` + a revision graph with a single head via an intentional branch + merge — pure Expand columns, a SEPARATE constraint revision, Validate, Contract, plus a FORWARD additive reconciliation-polling branch off the constraint revision merged back to one head — for the Lease evolution of `app.jobs`), an operational backfill script [`day48_lease_backfill.py`](../../projects/ai-backend-data-layer/api/day48_lease_backfill.py) (restartable `FOR UPDATE SKIP LOCKED`, off the migration), and tests [`test_day48_alembic.py`](../../projects/ai-backend-data-layer/api/test_day48_alembic.py). Static Alembic + fake-session tests were executed (40 passed) and the offline `alembic upgrade --sql` rendered the DDL (Python 3.10.12, Alembic 1.13.1, SQLAlchemy 2.0.29, pytest 7.4.3); **PostgreSQL runtime, integration, and production migration are NOT RUN** — see [projects/ai-backend-data-layer/README.md](../../projects/ai-backend-data-layer/README.md)
...
This lesson has **real static/offline evidence**: **33 tests passed** (Alembic revision-graph + migration-source
...
Day48 Alembic versioned schema evolution (Expand[cols] -> Constraints -> Backfill -> Validate -> Switch -> Contract)   <-- you are here
...
Safe evolution = Day36's phases, each SEPARATELY GATED by different evidence:
  ...
  EXPAND    add nullable cols ONLY, NO fabricated default, NO constraint -> the OLD/NEW COMPATIBILITY WINDOW
     |      (old Writers can still write a running-without-Lease row). Deploy FIRST and alone.
     v
  CONSTRAINTS add CHECK ... NOT VALID (triple coherence + jobs_running_requires_lease) in a SEPARATE revision,
     |      ONLY AFTER old Writers are DRAINED/ISOLATED. NOT VALID skips the legacy SCAN but ENFORCES every future
     |      write by ANY Writer version (an old Worker writing a running-without-Lease row is now REJECTED).
     v
...
`ADD CONSTRAINT jobs_lease_triple_coherent CHECK (...) NOT VALID` (the separate `0003_add_lease_constraints`
revision, applied after old Writers are drained) then, after Backfill, `VALIDATE CONSTRAINT ...` (the separate
`0004_validate_lease` revision). `NOT VALID` protects new writes from **every** Writer version — it is not a
"no-op for old code," which is why it lands only after the old write path is closed.

### Framework Connection

PostgreSQL `CHECK ... NOT VALID` + `VALIDATE CONSTRAINT`; the constraint revision (`0003_add_lease_constraints`,
applied after old Writers are drained) and the separate `0004_validate_lease` revision.
...
verified `'failed'`/`'cancelled'` to the **guarded terminal-recovery** path (state machine + Event + audit). An
...
`resolved_at`. A guarded `UPDATE` that affects **0 rows** (the Job no longer matches `running` + unowned) does **not**
...
missing column. They are added **forward** by a separate **additive** revision, `0006_add_reconciliation_polling`
(`ALTER TABLE app.job_lease_reconciliation ADD COLUMN ...` + a partial `WHERE resolution_status='open'` due-index). We
also must not simply append it after the Contract head, and we must not rewrite `0004`/`0005`'s parent: we cannot prove
those were never applied, and forcing a database still at `0003` to run Validate/Contract before it can get the polling
...
1h). That single write — no fabricated Lease, no requeue, no `app.jobs`/`job_status` touch, no Provider — makes the
record **not due**, so it is not re-selected in this loop and `run_reconciliation_resolution()` **terminates** even
with `max_batches=None`. Termination therefore rests on the **due-filter plus the forward backoff**, not on a mocked
empty batch. This is deliberately **polling, not Job retry and not Provider retry**: nothing external is re-invoked;
the resolver only re-checks, on a schedule, whether trusted ownership evidence has finally appeared. The record stays
`open` and still counts in `unresolved_running_without_lease`, so `unresolved` reaches 0 — the VALIDATE gate — only
after a **real** Lease write. A queued Job is not silently re-picked by the automatic loop; it is resolved, or
patiently re-polled, auditably, here.
...
The Contract revision (`0005_contract_legacy`) drops the Day42 legacy `result_object_key` only under explicit
...
**`alembic -x db_url=<url>` > env `DAY48_ALEMBIC_DATABASE_URL`**, with the `alembic.ini` `sqlalchemy.url` a
**non-credential placeholder used *only* for offline `--sql` rendering** (it selects the dialect and never
connects; it is **not** a production connection). Crucially, the placeholder is **not** an online connection
fallback: **online** mode **requires** a real external URL (`-x db_url` or `DAY48_ALEMBIC_DATABASE_URL`) and
**fails fast** with a no-credential error otherwise, while only **offline** rendering may use the placeholder. No
real connection string is ever committed. Two
...
`NOT VALID` means old Workers are unaffected

❌ "Add the running-requires-Lease CHECK in Expand; `NOT VALID` means old Workers keep working."
✅ `NOT VALID` skips only the one-time scan of legacy rows; it fully enforces the rule on every future
INSERT/UPDATE by any Writer version, so an old Worker updating a running-without-Lease Job is rejected. Pure
Expand (columns only) is the compatibility window; the strict constraint is a separate revision applied only
after old Writers are drained/isolated.

Why beginners think this: "NOT VALID" sounds like "not enforced."
How to remember: NOT VALID protects future writes regardless of who writes them.

Requeue an unknown Provider outcome to `queued`

❌ "If we can't tell what the Provider did, set the Job back to `queued` so it clears the unresolved count."
✅ A requeue clears the count without proving whether the Provider executed, charged, or produced an Artifact —
it double-executes paid work. An unknown outcome stays unknown/reconciliation; a verified `succeeded` goes
through the Day47 guarded completion UoW and a verified `failed`/`cancelled` through guarded terminal-recovery,
never a bare status flip.

Why beginners think this: `queued` looks like a safe retry.
How to remember: unknown is a state, not a requeue.
...
SQLAlchemy 2.0.29, pytest 7.4.3 → **33 passed**; install via `requirements-day48.txt`), plus an offline `alembic
...
14. env.py stays MINIMAL (DB config + Base.metadata + execution); Alembic control plane != FastAPI startup != Day47 UoW; app never self-migrates. DB URL resolves `-x db_url` > env DAY48_ALEMBIC_DATABASE_URL; the ini placeholder is OFFLINE-render ONLY (never an online fallback) and ONLINE fails fast without an external URL (no credentials committed).
...
Validation status: **33 static/offline tests** are **real executed evidence** — the Alembic revision graph +
...
1.13.1 / SQLAlchemy 2.0.29 / pytest 7.4.3 → **33 passed** with **no database connection**. But offline render and
