...
(The Day48 lesson has not started and no Day48 lesson file exists yet; see CURRICULUM.md and ROADMAP.md.
Day47 is recorded under Last Completed Lesson.)
...
Main Artifact:
Day47 async persistence boundary (projects/ai-backend-data-layer/api/day47-async-persistence-boundary-design.md) with runnable code (day47_async_uow.py) and fake-session tests (test_day47_async_uow.py) — process-scoped AsyncEngine/async_sessionmaker helpers, a request/Job-scoped AsyncSession, repositories that never commit, a UnitOfWork with explicit commit/rollback/close, the guarded `UPDATE ... WHERE job_status='queued' RETURNING` claim with zero-row stale/no-op, flush-before-dependent-write, a second guarded completion UoW, correlation-key-before-the-Provider-call recovery, and the integrated success/crash recovery drill (reuses the Day46 mapping; no schema redefinition, no global Session, no repo-owned commit, no Provider call inside a DB transaction)

Validation Boundary:
Day47 has REAL executed FAKE-SESSION control-flow evidence. Executed: python3 -m pip install -r requirements-day47.txt; python3 -m py_compile day47_async_uow.py test_day47_async_uow.py (passed); python3 -m pytest -q test_day47_async_uow.py -> 13 passed (Python 3.10.12, SQLAlchemy 2.0.29, greenlet 3.5.4, pytest 7.4.3, pinned in projects/ai-backend-data-layer/api/requirements-day47.txt). These prove UoW/repository CONTROL FLOW with a FAKE AsyncSession (explicit commit, rollback+close on exception/uncommitted exit, zero-row stale/no-op, flush-before-write, repos never commit, one shared Session per UoW, a fresh Session per UoW); a mock is NOT database proof. NOT RUN: PostgreSQL runtime rollback/transaction behavior (a real test would apply the independent Day42 raw SQL, force a failure, then prove via a NEW Session that the Job stays queued with no Attempt/Event); SQLite is NOT PostgreSQL evidence for this app-schema/PostgreSQL-typed contract; FastAPI/Worker integration, concurrent Workers, real Provider SDK/network, Object Storage, and production. Boundary preserved: PostgreSQL is durable authority; Day46 maps it, Day47 drives it, Day48 evolves it; long/paid Provider calls stay OUTSIDE the DB transaction; code rollback != durable-data rollback != external-side-effect rollback; unknown external outcome is a first-class recovery state, never blindly replayed.

Completed Work:

- Day47 classroom learning
- Day47 lesson document (LESSON_TEMPLATE_v2, v3.2 continuity + Day46->Day47 mental-model evolution)
- Day47 async persistence boundary design artifact + runnable code + fake-session tests (executed: 13 passed) and project README increment
- Day47 scope/ownership, repository-vs-UoW commit, explicit commit, guarded claim/stale-no-op, flush-vs-commit, IntegrityError, commit-exception unknown outcome, short-tx-vs-Provider, correlation-key-first, guarded-completion-vs-CHECK, failure-vs-unknown, detached-ORM-DTO, and evidence-levels exercises
- Day47 FastAPI cheat sheet append
- Day47 FastAPI interview notes append (incl. the three English "不知道" stages and their strong answers)
- Day47 repository status update
---

## Next

- Day48 — Alembic and Safe AI Backend Schema Evolution (Phase 4 — Production AI API Engineering)
