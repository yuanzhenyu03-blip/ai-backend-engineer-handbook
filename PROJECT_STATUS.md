...
Most recent classroom lesson: Day86 — Agent Security: Prompt Injection, Tool Abuse, Data Exfiltration and Sandboxing.
...
Artifact: `projects/ai-agent/src/agent_security.py`, 28 behavior tests, 3 grader tests, 25 version-1 seed
cases, deterministic example, design, classroom record and validation evidence. It adds application-bound
content trust/provenance, current Tool/Egress/Sandbox admission, exact tenant/resource/grant/approval/fence/
data checks, Fake effect ports, result-candidate verification, unknown-outcome reconciliation, required
security fan-in, bad-policy quarantine and append-only incident evidence. Evidence: CONCEPTUAL + STATIC +
EXECUTED_LOCAL_RUNTIME — Python 3.11.5, 31 Day86 / 474 cumulative tests, 25/25 Day86, 18/18 Day85,
...
PRODUCTION NOT RUN. Day87 is next.
...
Artifact: `projects/ai-agent/src/multi_agent_coordination.py`, 29 behavior tests, 3 grader tests, 18
version-1 seed cases, deterministic example, design, classroom record, validation evidence and update report.
It adds application-owned handoff acceptance, exact duplicate/conflict handling, bounded grant/allocation,
Outbox recovery, claim/lease/fence ownership, dispatch authorization, unknown-outcome reconciliation,
cancellation, result verification and required/optional fan-in. Evidence: CONCEPTUAL + STATIC +
EXECUTED_LOCAL_RUNTIME — Python 3.11.5, 32 Day85 / 443 cumulative tests, 18/18 Day85, 16/16 Day84 and
...
INTEGRATION_RUNTIME and PRODUCTION NOT RUN. Day86 is complete; Day87 is next.
...
`projects/ai-agent/src/context_memory.py`, 30 boundary tests, 3 grader tests, 16 version-1 seed cases,
deterministic example, design, classroom record and validation evidence. It separates permission-scoped,
lossy context representations from current durable Job/Attempt/Approval/Reservation/Outbox authority;
adds context assembly/manifests, bounded already-validated Tool-result views, source-span/revision-checked
compaction, protected result references and Day83-guarded rehydration. Evidence: CONCEPTUAL + STATIC +
EXECUTED_LOCAL_RUNTIME — Python 3.11.5, 33 Day84 / 411 cumulative tests, 16/16 Day84 seed cases,
...
`projects/ai-agent/src/human_control.py`, scenario fixtures, tests, example, seed eval, design and classroom
record. 53 Day83 / 378 cumulative local tests, 26/26 version-1 seed cases and mandatory checkpoint PASS on Python 3.11.5. CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME only.
Approval is exact and current-authorization-bound; interrupt cannot erase effects; unknown preserves original
...
`projects/ai-agent/` with `src/durable_agent_jobs.py`, `tests/test_day82_durable_agent_jobs.py`, the released
design and classroom record. The database owns durable lifecycle and recovery facts; a Checkpoint binds
Job/Step/Attempt, state version, fence and immutable execution bindings. Recovery rereads those facts before
classifying resume/retry/replan/reconciliation/repair/settlement/compensation/escalation. Unknown external
execution preserves the original operation identity and held reservation; committed unpublished Outbox intents
are redispatched at-least-once and consumer idempotency suppresses duplicate effects. Lease takeover advances
...
`projects/ai-agent/` with `src/agent_state_machine.py`, `tests/test_day81_agent_state_machine.py`, the released
design and classroom record. Verified facts and ordered guards—not model advice—produce a candidate transition;
an authoritative conditional apply boundary checks Job/Step/Attempt, state and fence and atomically models state
plus Step/token/cost reservations. Known prerequisites wait; unknown external outcomes preserve original identity
and held reservations in `PENDING_RECONCILIATION`; hard no-progress loops terminate with
...
`src/agent_loop.py`, `tests/test_day79_agent_loop.py`, the released design and classroom record. The
application-owned Controller returns a deterministic closed decision from trusted state; stable identities
make replay idempotent; terminal/stale/duplicate/non-continue paths invoke the Runtime zero times; only a new
...
`src/tool_governance.py`, `tests/test_day80_tool_governance.py`, the released design and classroom record. The
application owns a deterministic context-scoped Tool Capability Snapshot, permission-narrowed Schema,
immutable candidate binding and current Permission/Registry recheck; Framework translation cannot expand
authority; passing Day80 only enters existing Day74 generic and Day66 browser-specific Admission, while
Day78 retains execution/outcome and Day79 retains control. `202 Accepted` is not completion; pre-execution
...
Day72 is COMPLETE (classroom scope) — released the lesson `docs/fastapi/day72-provider-capabilities-and-the-replaceable-provider-adapter.md` and evolved the `projects/ai-agent/` Phase 7A Artifact (`src/provider_contract.py` + `src/provider_adapters.py` + `tests/test_provider_adapters.py` + `docs/DAY72_PROVIDER_ADAPTER.md` released design contract + `docs/day72-provider-adapter-classroom-draft.md`). DIRECT PREREQUISITES: Day71 (provider-independent runtime), Day53 (ProviderRequest -> ProviderOutcome seam), Day61 (real-HTTP Adapter foundation). Core model: the application owns a stable product contract; Providers offer only versioned capabilities behind a replaceable Adapter (Protocol + Registry) that translates syntax without silently weakening product meaning; capability admission runs before a paid call (incompatible -> CAPABILITY_ERROR + 0 calls); Capability Profiles are immutable versioned audit facts (drift -> disable/quarantine -> new revision); each Attempt persists its execution contract; Provider SUCCESS stays untrusted until validation + guarded completion; TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION; rollback stops future harm without erasing history. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 58 tests OK, Python 3.12.13; RecordingTransport + fixtures only). INTEGRATION_RUNTIME + PRODUCTION are NOT RUN (no real SDK/HTTP/Provider/PostgreSQL/credentials/paid call); Provider A/B are fictional fixtures; full Fake Provider/LLM regression suite = Day77. The final Chinese synthesis was Tech-Lead-supplied at the student's explicit request. Day73 is COMPLETE (classroom scope) — released the lesson `docs/fastapi/day73-prompt-contracts-prompt-versioning-and-compatibility.md` and evolved the `projects/ai-agent/` Artifact (`src/prompt_contracts.py` + `tests/test_prompt_contracts.py` + `docs/DAY73_PROMPT_CONTRACTS.md` released design contract + classroom draft). Core model: the prompt is an application-owned, versioned execution contract with immutable revisions + a lifecycle overlay, each Attempt durably binds its revision + renderer + parameter policy + application contract + input fingerprint + rendered hash, the current default governs only new planning, the LLM Runtime enforces a fail-closed pre-Provider gate, compatibility is directional + structural + semantic, and incident recovery uses disable/rollback/migration/re-release without rewriting history. Evidence: CONCEPTUAL + STATIC + EXECUTED_LOCAL_RUNTIME (`python3 -m unittest discover -s tests -v` -> 97 tests OK: 39 Day73 + 58 Day72 regression, Python 3.11.5); INTEGRATION_RUNTIME + PRODUCTION NOT RUN. Day74 is COMPLETE (classroom scope) — released `docs/fastapi/day74-structured-output-json-schema-and-function-tool-calling.md` and evolved `projects/ai-agent/` with `src/output_tool_contracts.py`, 34 Day74 tests, the released design contract and classroom record. Core model: untrusted Provider/tool candidates pass Parse -> strict Schema -> exact Registry -> trusted-context Authorization -> Semantics -> immutable Admission -> idempotent guarded Execution -> Outcome Verification -> guarded completion/reconciliation. Evidence: CONCEPTUAL + STATIC (`py_compile` + `mypy`) + EXECUTED_LOCAL_RUNTIME (131 total tests OK, Python 3.11.5); Python 3.12, full JSON Schema, INTEGRATION_RUNTIME and PRODUCTION NOT RUN. Day75–Day83 are COMPLETE at recorded classroom scope; Day84 is the current/next-up lesson.
...
Day83 — Human Approval, Interrupt and Escalation Boundaries (Phase 7B; guided classroom scope)

Completed Time: 2026-09-03

Main Artifact: `projects/ai-agent/src/human_control.py`; 53 Day83 / 378 cumulative local tests, 26/26 version-1 seed cases and mandatory checkpoint PASS on Python 3.11.5.
Independent final synthesis NOT ASSESSED; real integration NOT RUN. See Current Lesson for limitations.

## Superseded — Day60 Last Completed Lesson (archived)
...
- Day69 completed (classroom scope): `projects/n8n-workflows/` Day69 Risk-based Approval Gate contract (Approval Gate + exact binding + lifecycle + classified retry/error matrix + Secret boundary + current-state/append-only audit + smallest-safe-boundary Error Workflow + incident runbook) + the lesson. Day67/Day68 boundary unchanged. Human approval is RISK-BASED (authorized tenant role, not platform staff); validation != permission; a v7 approval cannot authorize v8 (stable task_id/correlation_id/tenant_id; new approval_id/event_id/publication operation_id/idempotency key); n8n timeout != Approval state (PENDING until backend expires_at); HTTP timeout = OUTCOME_UNKNOWN -> same operation_id + key -> query authenticated FastAPI -> classify (429/503|400/422|401 stop+classify (rotate only when established, else fix config)|403|409|terminal|unknown->PENDING_RECONCILIATION; NOT_FOUND is ambiguous — verify identity/routing/retention + evidence, reissue same key only if backend proves NEVER_ACCEPTED and retention still valid, else reconcile); Secrets only in the Credential Store (401 stopped+classified, never log the Authorization header); audit = current-state + append-only events committed atomically (not tamper-proof); delivery at-least-once (same event_id+same fingerprint=no-op; +different fingerprint=conflict); incident = contain -> revoke/rotate -> preserve -> scope -> classify -> cancel/reconcile/compensate -> verify -> regression -> controlled rollout (never delete/retro-approve/reverse external effects). Evidence: CONCEPTUAL_STATIC (design review); final Chinese synthesis taught directly by the Tech Lead at the student's request. NOT RUN: n8n runtime, FastAPI approval/publication integration, real approval/callback/approver auth, PostgreSQL Approval schema/migration/audit-events/Outbox, retry/backoff/error workflow, callback duplicate/fingerprint-conflict, credential-store/rotate/redaction, publication/notification target/reconciliation, Worker/Provider/Browser-Tool, rollback/kill-switch/canary, production; no exported workflow JSON. Day67's 400 and Day68's contract not reused. No secrets committed.
- Day70 completed (Phase 6 capstone): `projects/n8n-workflows/` DAY70_CAPSTONE.md + day70_capstone.py (standalone decision model: acceptance recovery, event dedupe/conflict, exact Approval binding, Publication recovery, credential classification, incident Task classification, polling boundary) + test_day70_capstone.py (21 tests; the classroom's 14 pre-fix tests superseded) + a Secret-free importable day70_minimal_acceptance_workflow.json (two independent auth boundaries, Credential Store references only, `{document_ids, business_input:{report_scope}}` body; not a captured post-run export). n8n orchestrates; FastAPI/PostgreSQL owns durable truth (Day67–Day69 boundary unchanged). Evidence: EXECUTED_LOCAL_RUNTIME — decision model + workflow static-contract tests (classroom pre-fix suite Python 3.11.5 = 14 passed, SUPERSEDED; Python 3.9.6 -> pytest missing = NOT RUN; updating agent FIXED suite Python 3.10.12 = 21 passed + affected Day59 fingerprint test 12 passed; Python 3.12 NOT RUN) + a real n8n inspection. INTEGRATION_RUNTIME (in class, not re-run by the agent) for the PRE-FIX workflow: a bounded real, authenticated n8n -> FastAPI/Uvicorn -> PostgreSQL SERVICE-call ACCEPTANCE slice (202 + idempotency_replayed=true on redelivery + new-connection jobs=1/outbox=1/document_links=1 + 400 on invalid) — acceptance boundary only; it did NOT prove inbound caller->n8n auth and is NOT evidence for the FIXED workflow (NOT RERUN; the different-report_scope 409 is proven only by the Day59 fingerprint contract test). NOT RUN: inbound Webhook authentication (external caller->n8n; source reference only, NOT CONFIGURED at run time), budget reservation, correlation_id persist/propagate, Polling/Callback/Approval/Publication/Error-Workflow runtime, Worker/Outbox/Provider/Browser-Tool participation, verified Artifact, credential rotation/exposure review, rollback/kill-switch/canary, Python 3.12, captured n8n post-run export, production. One run != cumulative integration != production; Day59–Day69 evidence is a prerequisite, not Day70 validation. No secrets committed. Day70 closes Phase 6.
- Day71 completed (Phase 7A foundations; classroom scope): the released lesson `docs/fastapi/day71-llm-application-architecture-tokens-context-sampling-and-model-failure-modes.md` + the `projects/ai-agent/` Phase 7A foundations (`docs/DAY71_FOUNDATIONS.md` released static provider-independent design contract: architecture boundary, token/context budget, overflow policy, chunk/aggregation contract, provider-independent vs provider-specific split, sampling policy, layered validation + failure taxonomy, recovery boundary, Job/Attempt, guarded completion, rollback; + `docs/day71-llm-runtime-foundations-classroom-draft.md`) + cheat/interview appends. Day70 -> Day71 is a PHASE_TRANSITION (Phase 6 -> Phase 7A); technical roots Day53-Day61; n8n is NOT a Day71 dependency. Core model: the application owns a Provider-independent contract; a token is model-specific and the context window is a finite shared budget; overflow policy (TRUNCATE/REJECT/SUMMARIZE-lossy/CHUNK) is a product decision; sampling is selection behaviour, not factual control (deterministic != reproducible != true); validation is layered (transport->schema->evidence->policy->guarded completion) and HTTP 200 + valid schema do not prove correctness; failure taxonomy (hallucination/unsupported, contradicted, format, truncation, timeout-unknown, provider-failure) separates root cause from symptom; timeout = TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION (no blind retry); Job vs Attempt (new call = new Attempt, never overwritten); guarded completion needs a live Job + the exact authorized Attempt (stale/late = zero-effect); rollback stops future harm, never erases history. Evidence: CONCEPTUAL (completed) + STATIC (PASS). NOT RUN: EXECUTED_LOCAL_RUNTIME (no runtime code/tests), INTEGRATION_RUNTIME (no Provider/DB/external process), PRODUCTION. No real or paid Provider call; no secrets. The final Chinese Mental Model synthesis was Tech-Lead-supplied at the student's explicit request. Day53-Day61 runtime evidence is a prerequisite, not Day71 validation.
- Day72 completed (Phase 7A; classroom scope): the released lesson `docs/fastapi/day72-provider-capabilities-and-the-replaceable-provider-adapter.md` + the evolved `projects/ai-agent/` Artifact (`src/provider_contract.py`: ProviderOutcomeKind surface, ApplicationRequest, versioned CapabilityProfile, AttemptExecutionContract, admit_capability, ProviderAdapter Protocol, ProviderRegistry, ProviderSelectionPolicy; `src/provider_adapters.py`: RecordingTransport + Provider A/B concrete adapters + dispatch_attempt; `tests/test_provider_adapters.py`: 58 tests; `docs/DAY72_PROVIDER_ADAPTER.md` released design contract + `docs/day72-provider-adapter-classroom-draft.md`; README updated) + cheat/interview appends. DIRECT PREREQUISITES Day71 + Day53 + Day61. Core model: the application owns a stable product contract; Providers offer only versioned capabilities; capability admission before a paid call (incompatible -> CAPABILITY_ERROR + 0 calls, never lowest-common-denominator); Capability Profile is an immutable versioned audit fact (drift -> disable/quarantine -> new revision); the Adapter does bidirectional translation by semantic equivalence (two output-limit states -> TRUNCATION), keeps SDK/wire types inside, preserves a stable failure taxonomy, and does not hide retries/switch Providers/decide Job terminal state (new call = new Attempt); Provider wire validation = Adapter, application evidence validation = Runtime; each Attempt persists its execution contract (current config governs new calls; bound version interprets an already-issued call; never rewritten); replaceability = equivalent application semantics via a Protocol + Registry (fail closed on DISABLED/QUARANTINED); client option = selector, server allowlist = authority; disabled profile -> BLOCKED_PROFILE_DISABLED (0 calls, A1 not rewritten); Provider SUCCESS untrusted until schema/evidence/policy + guarded completion; TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION; rollback stops future harm without erasing history. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (58 in-process tests OK, Python 3.12.13; RecordingTransport + fixtures only). NOT RUN: INTEGRATION_RUNTIME (no real SDK/HTTP/Provider/PostgreSQL/external process), PRODUCTION (no credentials/customer data/paid call). Provider A/B are fictional fixtures; capabilities are versioned facts; full Fake Provider/LLM regression suite = Day77. The final Chinese Mental Model synthesis was Tech-Lead-supplied at the student's explicit request.
- Day73 completed (Phase 7A; classroom scope): the released lesson `docs/fastapi/day73-prompt-contracts-prompt-versioning-and-compatibility.md` + the evolved `projects/ai-agent/` Artifact (`src/prompt_contracts.py`: PromptContractRevision + PromptLifecycle (fail-closed UNKNOWN) + PromptContractRegistry (current default for new planning only), VariableSpec/MessageTemplate + deterministic renderer + fingerprint/rendered-hash, AttemptPromptBinding + InMemoryAttemptPromptStore (guarded compare-and-set), prepare_dispatch pre-Provider gate, backward_incompatibilities + apply_migration, classify_timeout_unknown + interpret_late_response; `tests/test_prompt_contracts.py`: 39 tests; `docs/DAY73_PROMPT_CONTRACTS.md` released design contract + `docs/day73-prompt-contracts-classroom-draft.md`; README updated) + cheat/interview appends. DIRECT PREREQUISITES Day72 + Day71 + Day53 + Day54. Core model: a Prompt Contract is an application-owned, versioned execution contract (not a string, not a Provider capability); immutable revisions + a separate lifecycle overlay; each Attempt durably binds revision + renderer version + independently-versioned parameter policy + application contract + input fingerprint + rendered-message hash; the current default selects only for a NEW Attempt and never reinterprets history; the LLM Runtime enforces a pre-Provider gate (binding mismatch stays PLANNED with 0 calls; disabled bound revision -> BLOCKED_PROMPT_DISABLED with 0 calls; unknown/incompatible/invalid-variables/input-fingerprint/rendered-hash mismatch fail closed); compatibility is directional + structural + semantic (optional-with-default OK; removing any formerly accepted variable or application contract is breaking; rename-without-alias breaking; guarantee weakening breaking; unknown policy + alias conflict fail closed); audit keeps minimal evidence, full prompts only as protected finite-retention artifacts (a hash is integrity, a ref is a pointer); incident recovery uses disable/rollback/migration/re-release without rewriting history; TIMEOUT_UNKNOWN -> reconciliation, a valid late response is interpreted with the BOUND revision, a stale/terminal/superseded one never overwrites. Evidence: CONCEPTUAL + STATIC (py_compile) + EXECUTED_LOCAL_RUNTIME (97 in-process tests OK: 39 Day73 + 58 Day72 regression, Python 3.11.5; in-memory stores + pure functions only). NOT RUN: INTEGRATION_RUNTIME (no real SDK/HTTP/Provider/PostgreSQL/queue/worker/encryption/protected-artifact storage), PRODUCTION. The provider_calls counter models an in-process boundary crossing only; Day72 tests are prerequisite evidence, not Day73 validation. The final Chinese Mental Model synthesis was Tech-Lead-supplied at the student's explicit request.
