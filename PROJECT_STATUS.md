...
Day72 — Provider Capabilities and the Replaceable Provider Adapter (Phase 7A) — next up.

Most recently released: Day71 — LLM Application Architecture, Tokens, Context, Sampling and Model Failure Modes (Phase 7A)

Status:
Day71 is COMPLETE (classroom scope) — released the lesson `docs/fastapi/day71-llm-application-architecture-tokens-context-sampling-and-model-failure-modes.md` and the `projects/ai-agent/` Phase 7A foundations (`docs/DAY71_FOUNDATIONS.md` + `docs/day71-llm-runtime-foundations-classroom-draft.md`). Day70 -> Day71 is a PHASE_TRANSITION (Phase 6 -> Phase 7A); technical roots Day53-Day61; n8n is NOT a Day71 dependency. Evidence: CONCEPTUAL + STATIC only; EXECUTED_LOCAL_RUNTIME / INTEGRATION_RUNTIME / PRODUCTION are NOT RUN; no runtime code, no real or paid Provider call, no secrets. Day72 (Phase 7A) is the current/next-up lesson.
