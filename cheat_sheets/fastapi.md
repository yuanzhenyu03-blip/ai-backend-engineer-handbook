...
Retry = classified recovery, not replay. `HTTP timeout -> OUTCOME_UNKNOWN`. Keep operation_id + idempotency key (`approval-301:publish-report:artifact-v7`), query authenticated FastAPI status/reconciliation: SUCCEEDED->no republish; PROCESSING->observe; FAILED_TERMINAL->no blind retry; PENDING_RECONCILIATION->reconcile; NOT_FOUND->AMBIGUOUS, do NOT immediately reissue. Errors: 429/transient503->backoff+jitter+Retry-After; write timeout->query/reconcile first; 400/422->fix no auto-retry; 401->stop+CLASSIFY (rotate only if expiry/compromise/revocation/invalid established; else fix config for audience/issuer/scheme/missing-header/endpoint/clock-skew), verify, controlled retry; 403->stop+investigate; 409 idempotency-meaning conflict->stop+investigate; rejected/expired approval->business terminal; unknown external->PENDING_RECONCILIATION. NOT_FOUND (wrong identity/routing, retention passed, expired idempotency record, store-vs-provider inconsistency, or still-unknown): verify identity/routing/retention + audit/DB/logs/provider evidence; reissue with the SAME key ONLY when backend proves NEVER_ACCEPTED/NOT_STARTED AND idempotency retention still valid; else PENDING_RECONCILIATION. `same idempotency key != unconditional proof of safety` (an expired record can still duplicate a publish / add cost).

Secrets: workflow=credential REFERENCE; Credential Store/Secret Manager=real secret; runtime=controlled injection; logs/audit/export/evidence-pack = NEVER Token/Authorization header/API key/cookie/private key/raw payload/tenant content. 401 -> stop, preserve safe evidence, CLASSIFY (rotate only when expiry/compromise/revocation/invalid established; else fix config), revalidate approval/version, verify, controlled retry on same operation_id (never blind retry / never log the header; the Day69 scenario's expired credential makes rotation the scenario-specific recovery, not the universal 401 response).
...
## Day70 — n8n + FastAPI + AI Tool Integration Capstone and Interview (Phase 6 close)

Core: integrate Day67–Day69 into ONE failure-aware cumulative path; run the maximum feasible REAL n8n->FastAPI->PostgreSQL acceptance slice; keep strict evidence tiers. Boundary UNCHANGED: n8n orchestrates/maps/waits/branches/authenticated-calls (history = orchestration evidence); FastAPI authenticates/authorizes + enforces legal transitions + idempotency; PostgreSQL owns durable Task/Approval/Publication truth. Do NOT assign authentication to PostgreSQL.

Honest 202: only after the acceptance bundle commits. Day59 gate = Job/Task + Outbox intent + request fingerprint (64-char) + verified Document link, read from a NEW DB connection. Budget reservation belongs in the full production bundle but is ABSENT from current Day59 impl -> budget NOT RUN. Lost acceptance response -> OUTCOME_UNKNOWN: n8n preserves request/operation/idempotency identity + queries authenticated FastAPI (n8n cannot write PENDING_RECONCILIATION); reissue same key ONLY after authoritative NEVER_ACCEPTED + still-valid idempotency retention. NOT_FOUND/404 is AMBIGUOUS (wrong tenant/auth/route/env, replica delay, retention, wrong key type, or true absence) — verify authoritative facts; same key != unconditional retry permission.

Observe: every Poll queries the SAME task_id with bounded backoff + observation deadline. QUEUED -> n8n may wait/re-query/send non-authoritative progress; may NOT start a Worker, mutate state, fail the Task, or publish. Observation failure / disappearing n8n execution changes NO durable state; a new execution resumes observation of the same task_id (no new business command). A 503 while verifying an Artifact -> retry the authenticated OBSERVATION only (backoff+jitter), never Browser/Provider execution; deadline exhaustion -> reconciliation.

Approval exact binding: n8n requests/waits/branches; FastAPI authorizes the tenant approver + ATOMICALLY persists decision + audit event. Approval = exact tenant + authorized actor + exact action + exact Artifact/version + exact policy + bounded lifetime. v7 approval != v8. v7->v8: stable tenant_id/task_id/correlation_id; NEW artifact_version/approval_id/approval event_id/publication operation_id/idempotency key; REVALIDATE policy/action/actor.

Delivery vs identity: same event_id + same fingerprint = idempotent no-op (one business event, many receipts); same event_id + different fingerprint = conflict/investigation, no action. Publication timeout = OUTCOME_UNKNOWN: query authoritative operation status + strictly-matching external receipt -> SUCCEEDED (no republish) / PROCESSING (observe) / FAILED_TERMINAL / PENDING_RECONCILIATION / NOT_FOUND (verify before any reissue). 401 -> stop + CLASSIFY: rotate only when expired/revoked/compromised/leaked/invalid established; fix config for audience/issuer/scheme/header/endpoint/clock-skew.

Incident (bad release duplicated Approvals/Publications + leaked Authorization header): contain -> stop Error-Workflow replay + backend publication kill switch -> IMMEDIATELY revoke/rotate the leaked credential (not merely replace config), verify old rejected, no traffic yet -> preserve evidence -> scope (padded window + in-flight; join workflow/release/execution -> tenant/request/task/correlation -> Attempt/Provider -> Approval/event -> Artifact/version -> Publication operation/idempotency/receipt; credential exposure window is separate) -> classify (published-without-approval -> preserve SUCCEEDED + policy-violation + compensate, never retro-approve; provably-unstarted -> guarded FastAPI durable cancellation, 0 rows -> reclassify; Provider-dispatched unknown -> PENDING_RECONCILIATION) -> verify -> regression -> controlled rollout (synthetic->test tenant->canary->allowlist->gradual; stop on approval bypass/duplicate publication/event conflict/Provider amplification/auth spike/reconciliation growth/identity mismatch/audit gap/Secret exposure). Rollback stops FUTURE harm only. When n8n history says Publication failed but a strict external receipt proves success -> record SUCCEEDED + record ack/orchestration failure separately.

Evidence tiers (four-tier taxonomy): architecture reasoning + static config review = CONCEPTUAL_STATIC; pure Python decision-model + workflow static-contract tests (`day70_capstone.py` via `test_day70_capstone.py`) + real n8n inspection = EXECUTED_LOCAL_RUNTIME (classroom pre-fix suite Python 3.11.5 = 14 passed, SUPERSEDED; Python 3.9.6 -> pytest missing = NOT RUN not a failure; updating agent FIXED suite Python 3.10.12 = 21 passed + affected Day59 fingerprint test = 12 passed; Python 3.12 NOT RUN); a bounded real, authenticated n8n->FastAPI SERVICE-call ACCEPTANCE slice for the PRE-FIX workflow (top-level report_scope; no inbound Webhook auth) = INTEGRATION_RUNTIME (202 + idempotency_replayed=true on redelivery + new-connection jobs=1/outbox=1/document_links=1 + 400 on invalid) — proves the acceptance boundary ONLY, does NOT prove inbound caller->n8n auth, and is NOT evidence for the FIXED workflow (report_scope in business_input, three-field IF, inbound headerAuth reference), whose real integration is NOT RERUN (the different-report_scope 409 is proven only by the Day59 fingerprint contract test). NOT RUN: correlation_id persist/propagate, Polling/Callback/Approval/Publication/Error-Workflow runtime, Worker/Outbox/Provider/Browser-Tool participation, verified Artifact, real credential rotation/exposure review, rollback/kill-switch/canary, production. Tests/fakes never auto-upgrade to INTEGRATION_RUNTIME; one run != production. Day59–Day69 evidence is a prerequisite, not Day70 validation. No secrets/tokens/real URLs/tenant data/Provider payloads committed. Day70 closes Phase 6; Day71 = phase transition (Day53–Day61 foundations), not an n8n dependency.

Related: [Day70 lesson](../docs/fastapi/day70-n8n-fastapi-ai-tool-integration-capstone-and-interview.md) · [Day70 capstone](../projects/n8n-workflows/DAY70_CAPSTONE.md) · [n8n-workflows project](../projects/n8n-workflows/README.md) · Previous: [Day69 lesson](../docs/fastapi/day69-human-approval-retry-secrets-audit-and-error-workflows.md)


## Day71 — LLM Application Architecture, Tokens, Context, Sampling and Model Failure Modes (Phase 7A)

Phase transition: Day70 (Phase 6) is the chronological previous lesson but NOT the technical prerequisite. Day71's technical roots are Day53 (Provider boundary + structured output) -> Day54 (streaming/lifecycle) -> Day56 (resilience/token cost/backpressure) -> Day57 (fake Provider/contract tests) -> Day58 (observability/evidence) -> Day61 (Provider Adapter HTTP evidence). n8n has NO direct dependency on Day71.

Architecture: Business request -> Provider-INDEPENDENT application contract -> LLM Application Runtime (token/context budget · input transform · sampling policy · validation/failure classification) -> Provider Adapter -> Provider-specific API. The application owns the contract; the Adapter translates VERSIONED capabilities/failures and must NEVER silently weaken `research_claims.v1`.

Tokens/context: a token is a MODEL-SPECIFIC unit (not fixed chars/words/bytes). `input = system + user + documents + overhead`; `input + reserved output + safety margin <= effective context limit`. Reserved output = capacity, not a promise. Provider token counts/context sizes/params are VERSIONED capabilities, never permanent facts. Admission check runs BEFORE a paid call.

Overflow policy (product decision from the contract): TRUNCATE (lossy, can destroy evidence) | REJECT (no call when contract can't be met) | SUMMARIZE (LOSSY) | CHUNK (stable doc/chunk identity, reconcile+aggregate). Evidence-critical + exact citations -> CHUNK; summarize only when loss is acceptable. Classroom trajectory: `摘要` -> exact-evidence constraint -> `分块`.

Chunk/aggregation: chunking is an input-division strategy — the number of Provider calls depends on the processing plan, per-call context budget, evidence-completeness and Provider capability, not on the word "chunk" (no universal "one chunk = one call" rule). In the Day71 overflow scenario the three chunks each need independent processing and can't share one context, so it's normally three processing calls + one bounded aggregation call (one big call does NOT solve overflow). Candidate = {claim_id, claim, document_id, chunk_id, evidence_excerpt, scope, status=CANDIDATE}. Aggregation input = bounded candidates + evidence excerpts + sources (NEVER raw docs again). Outcome ∈ RESOLVED | CONFLICT | INSUFFICIENT_EVIDENCE; conflicts keep provenance -> reconcile (never concatenate as fact).

Provider-independent contract vs Provider-specific capability: capability (VERSIONED) = context limit, native structured-output support, sampling params, finish/error evidence; contract (STABLE) = app request/result/failure MEANING. Provider can't meet contract -> explicitly supported path OR `PROVIDER_CAPABILITY_ERROR`; never silently accept ordinary text.

Sampling: selects next token from a probability distribution; `temperature` concentrates/relaxes candidate probabilities, `top_p` restricts candidate mass; semantics are Provider/model-specific. Sampling = SELECTION behaviour, NOT factual control. Deterministic (same complete conditions -> same output) ≠ Reproducible (procedure rerun/reviewed; not byte-identical for a managed model; a seed helps repeatability, not a universal guarantee) ≠ True.

Layered validation: transport -> schema -> evidence -> policy -> guarded completion. HTTP 200 + valid schema do NOT prove factual correctness. Failure taxonomy: HALLUCINATION_OR_UNSUPPORTED_EVIDENCE | CONTRADICTED_EVIDENCE (citation exists but contradicts claim; caught at evidence gate) | FORMAT_FAILURE | TRUNCATION | TIMEOUT_UNKNOWN | PROVIDER_FAILURE. Root cause vs symptom: output-budget exhaustion = primary TRUNCATION; incomplete JSON = derived FORMAT_FAILURE.

Recovery: retry (NEW external execution) | reject (refuse execution/result) | reconciliation (gather evidence for UNKNOWN outcome). Timeout after dispatch = TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION, hold original Attempt identity + cost uncertainty, NO blind retry; Adapter request/status evidence supports reconciliation. Late result passes the SAME gates; schema-valid-but-unsupported late = COMPLETED_INVALID.

Job vs Attempt + guarded completion: job_id = stable business identity; attempt_id = one execution; new Provider call = NEW Attempt (old Attempts never overwritten/deleted). Guarded completion needs a LIVE Job + the EXACT currently authorized Attempt; a stale/late result = zero-effect (no Job/Artifact/success-Event/cost overwrite). Example: Job SUCCEEDED, A1 TRUNCATED (retained), A2 SUCCEEDED.

Rollback/incident: configuration rollback stops FUTURE harm; it never undoes Provider calls, cost, outputs, or history. Bad output-budget policy v2 -> roll back (stop new v2 traffic; safe v1 may continue) -> preserve evidence -> scope affected set by policy/release version + time window + failure evidence -> per-Job eligibility -> controlled NEW Attempts only. No bulk success, no unconditional retry, no history deletion.
...
Direct prerequisites: Day72 (replaceable Provider Adapter + versioned capability admission) + Day71 (provider-independent runtime) + Day53 (Provider boundary / untrusted success) + Day54 (Attempt lifecycle / timeout-unknown / late-result identity binding). Day73 adds the application-owned prompt boundary that runs BEFORE Day72 capability admission.

Core boundary: a Prompt Contract is an APPLICATION-OWNED, versioned execution contract — NOT a mutable string and NOT a Provider capability declaration. A published `PromptContractRevision` is immutable (prompt_contract_id + revision + ordered message templates/roles + required/optional variables + input schema + semantic guarantees like `citations_required` + compatible application contracts + renderer_version). Operational status is a SEPARATE lifecycle overlay: ACTIVE / DISABLED / QUARANTINED, and an unregistered revision is UNKNOWN — every caller fails closed (never assume ACTIVE).

Durable Attempt binding (immutable): prompt_contract_id + revision, renderer_version, parameter_policy_id + parameter_policy_revision (model params are versioned INDEPENDENTLY, not hidden in the prompt), application_contract, input_fingerprint (canonical SHA-256 over validated inputs), rendered_message_hash (SHA-256 over ordered rendered messages), optional rendered_artifact_ref. The CURRENT DEFAULT selects a revision for a NEW Attempt only; it NEVER reinterprets an already-planned or dispatched Attempt.

Ownership: application business authorization (a prompt variable is NEVER authorization evidence) · LLM Runtime (selection/binding/lifecycle/variable+schema validation/compatibility/deterministic rendering/binding verification/guarded transitions/reconciliation) · Model Parameter Policy (temperature/limits, versioned independently) · Provider Adapter (provider translation only — never chooses/rewrites the revision) · output/business validation (a transport SUCCESS is an untrusted candidate).

Pre-Provider Runtime gate (`prepare_dispatch`), each failure = provider_calls=0: caller binding == authoritative store binding (else BINDING_MISMATCH, Attempt stays PLANNED) -> state PLANNED -> bound revision registered (else UNKNOWN_PROMPT) -> lifecycle ACTIVE (UNKNOWN -> fail closed; DISABLED/QUARANTINED -> guarded PLANNED -> BLOCKED_PROMPT_DISABLED) -> application-contract compatible (else INCOMPATIBLE_CONTRACT) -> variables valid + re-render hash == bound hash (else VARIABLE_INVALID / RENDER_HASH_MISMATCH) -> guarded PLANNED -> DISPATCHED, READY (provider_calls=1, models an in-process boundary crossing only). A dispatch-vs-binding mismatch is rejected BEFORE a valid transition (stays PLANNED) — distinct from BLOCKED_PROMPT_DISABLED (the bound revision itself is disabled).

Compatibility is DIRECTIONAL and both STRUCTURAL and SEMANTIC. Backward = a newer revision accepts valid OLD inputs and preserves old guarantees. Forward = an older runtime safely handles/REJECTS unknown fields (never silently accepts). Adding an optional variable with a deterministic default = compatible; renaming `evidence`->`sources` without an alias/migration = structural break; changing "citations required" -> "optional" = semantic break even if schema is unchanged; unknown `tenant_policy` value = fail closed; conflicting alias+replacement = reject (AliasConflictError). Migration is explicit, non-mutating, auditable — never rewrites a historical Attempt.

Audit: minimal evidence only (ids/revisions/lifecycle decision/input fingerprint/rendered hash/dispatch+outcome ids/state transition + failure class). A full rendered prompt is sensitive -> protected artifact (encryption, tenant-aware authz, access audit, finite retention). A hash is integrity evidence, NOT encryption; a rendered_artifact_ref is a pointer, NOT authorization.

Incident verbs (distinct): DISABLE (stop a revision from new dispatch) · ROLLBACK (select an older safe revision for new planning) · MIGRATION (transform compatible data without rewriting history) · RE-RELEASE (publish a corrected new immutable revision). Bad `v2` weakened a guarantee -> disable v2, rollback default to v1, scope by prompt_contract_id+revision / renderer+parameter-policy version / release-time window / durable dispatch+outcome evidence, classify per Attempt (PLANNED-never-dispatched -> block/re-plan; dispatched-successful -> untrusted candidate + revalidate guarantee; definitely-invalid -> explicit failure; TIMEOUT_UNKNOWN -> PENDING_RECONCILIATION no blind retry; valid-late -> interpret with the BOUND revision; stale/terminal/superseded -> never overwrite), re-release v3 under controlled rollout.
