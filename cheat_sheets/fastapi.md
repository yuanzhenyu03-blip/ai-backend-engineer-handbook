...
Evidence tiers (four-tier taxonomy): architecture reasoning + static config review = CONCEPTUAL_STATIC; pure Python decision-model tests (`day70_capstone.py` via `test_day70_capstone.py`) + real n8n inspection = EXECUTED_LOCAL_RUNTIME (classroom Python 3.11.5 = 14 passed; Python 3.9.6 -> pytest missing = NOT RUN not a failure; updating agent Python 3.10.12 = 14 passed; Python 3.12 NOT RUN); bounded real n8n->FastAPI->PostgreSQL ACCEPTANCE slice = INTEGRATION_RUNTIME (202 + idempotency_replayed=true on redelivery + new-connection jobs=1/outbox=1/document_links=1 + 400 on invalid) — proves acceptance boundary ONLY. NOT RUN: correlation_id persist/propagate, Polling/Callback/Approval/Publication/Error-Workflow runtime, Worker/Outbox/Provider/Browser-Tool participation, verified Artifact, real credential rotation/exposure review, rollback/kill-switch/canary, production. Tests/fakes never auto-upgrade to INTEGRATION_RUNTIME; one run != production. Day59–Day69 evidence is a prerequisite, not Day70 validation. No secrets/tokens/real URLs/tenant data/Provider payloads committed. Day70 closes Phase 6; Day71 = phase transition (Day53–Day61 foundations), not an n8n dependency.
