...
Timeout: a POST-ACTION timeout (request sent, no response) = UNKNOWN_OUTCOME (may have executed) -> reconcile, NOT retry. Only a PROVEN non-start = SAFE_TO_RETRY. Reconcile the ORIGINAL action by strict Day64 identity (client_request_id/report_id/verified export_id) + a server status/audit lookup — NEVER a broad URL+200; a record with a different client_request_id is STILL_UNKNOWN. `classify_timeout`, `reconcile_unknown`.
