...
Pre-call execution gate (a PAID side effect is claimed FIRST): `execute_job` order = claim eligibility -> Provider call -> process -> guarded completion. Only a RUNNING Job may START a new call; SUCCEEDED/FAILED (terminal) and PENDING_RECONCILIATION (awaiting a late result) -> `PRECALL_BLOCKED` with **transport calls == 0** (never re-trigger a paid call just to be NOOP'd at the DB write). Then bind to contract: `bind_request_to_contract` derives model/schema/version/task/profile from the persisted `ExecutionContract` (caller cannot pick them); a mismatch -> `CONTRACT_MISMATCH` before any transport call; max tightened to `min(request, bound, model hard cap)`, never enlarged.

Late-outcome ingestion (PATH B): a result that arrives after a timeout is handled by `ingest_late_outcome(outcome, job_id, correlation_id)` — it makes NO adapter/transport call, validates job_id + correlation (+ provider_request_id) against the persisted contract, then runs guarded completion (mismatch -> `LATE_OUTCOME_REJECTED`; terminal Job -> `COMPLETION_NOOP`, no overwrite). Calling `execute_job` a second time is NOT late-result handling (it issues a second paid Provider call). Day54 owns the real callback/streaming/cancellation protocol.
...
Validation: REAL Pydantic v2 strict validation + an in-memory Adapter->Validator->Completion model with an INJECTED FAKE transport (Python 3.10.12, pydantic 2.5.0, pytest 7.4.3 -> 36 passed). Proves the validation gate + application control flow ONLY. NOT the real `openai` SDK/network/Provider, NOT PostgreSQL/Redis/Celery Worker, NOT FastAPI wire/integration/production. Day54 streaming/disconnect/cancellation, Day55 Celery, Day56 retry/backoff not implemented. No real api_key/prompt/Document content/Provider response persisted or logged.
