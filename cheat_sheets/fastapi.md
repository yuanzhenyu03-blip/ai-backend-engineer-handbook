...
Hardened (review round 1): finalize is legal-state + expiry guarded (only uploading/verifying; INITIATED/FAILED/EXPIRED and cleanup-claimed rows rejected). A transient scanner outage takes a persistent verification hold (modeled `verifying` + `verification_hold_until`) so cleanup (KEEP_VERIFICATION_HOLD) cannot delete a retrying object; `claim_cleanup` commits `expired` first so the completion/cleanup race is deterministic. Object adapter is create-only + version-history (exact-version inspect/delete); Document-create + verified flip is one MODELED atomic UoW (mid-tx failure leaves neither fact). Server owns bucket+key (+ bound version); completion rejects client-supplied identity (REJECTED_IDENTITY) and verifies observed bucket/key/version/size/sha256/content-type.
Schema honesty: the published upload_sessions allowlist has NO `verifying`; the hold is MODELED in-memory. The REAL schema needs a Day48-safe FORWARD migration (add a `verifying` status via a branch revision, or a verification-hold/lease table) — not implemented here; never a rewrite of published Alembic history.

Validation: FAKE in-memory Object Storage adapter — application CONTROL FLOW only, incl. a MODELED atomic UoW (Python 3.10.12, pytest 7.4.3 -> 35 passed; stdlib-only module). **NOT** real presigned/checksum/multipart/versioning semantics, **NOT** PostgreSQL runtime, **NOT** a real Object Storage integration, **NOT** production. Day50 Outbox / Day51 JWT / Day52 authorization / Day55 Celery / real Provider NOT implemented.
