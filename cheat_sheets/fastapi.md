...
Polling (observation of an existing Task; retrying a poll != retrying the job): `Wait(bounded interval/backoff) -> GET /tasks/{task_id} -> Switch(authoritative status)`: queued/running -> backoff -> poll same task_id; succeeded -> consume verified artifact_ref; failed/cancelled/expired -> terminal; timeout/429/503/invalid -> retain task_id, retry or reconcile. Bounds: interval/backoff + observation deadline + max attempts + terminal stops. 503 = observation channel down, NOT task failure; 404 != auto business failure (identity/tenant/retention/integration — investigate). Audit logs = investigation evidence, NOT authoritative state/recovery.
...
Identities: `request_id` (one logical acceptance command, stable across lost-response retries) · `task_id` (durable backend Task) · `correlation_id` (stable business-chain association, NOT auth/authz) · `event_id` (stable callback identity, for dedupe) · `task_version` (monotonic ordering/conflict) · `trace_id`/poll-attempt (one concrete attempt, may change).

Callback = AT-LEAST-ONCE (never claim exactly-once): authenticate -> validate schema -> match task_id+correlation_id -> dedupe stable event_id -> task_version ordering (reject stale/conflicting) -> verify legal transition -> optional authoritative FastAPI confirm -> ONE idempotent downstream action (e.g. `event_id:publish-report`). n8n history is NOT the idempotency store. Correlation mismatch (right task_id, wrong correlation_id) -> do NOT publish, preserve safe metadata, query FastAPI, reconcile; correlation matching != authentication. Ordering: incoming version < processed -> stale no-op/ack; == -> identical=idempotent no-op, conflicting=integration error; > -> verify legal transition. `arrival order != business-state order`.
