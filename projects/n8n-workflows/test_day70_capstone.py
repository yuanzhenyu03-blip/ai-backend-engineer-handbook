...
across the six capstone decision areas + the polling/observation boundary. Fourteen deterministic tests.
...
def _approval(**over):
    d = dict(approval_id="approval-301", tenant_id="tenantA", task_id="task-8421", artifact_version="v7",
             action="publish_report", approver_actor="user-9", approver_role="Compliance",
             policy_version="p1", expires_at=NOW + 100, decision="APPROVED", decided_at=NOW)
    d.update(over)
    return Approval(**d)
...
def test_approval_v7_cannot_authorize_v8_or_other_action():
    a = _approval(artifact_version="v7", action="publish_report")
    assert approval_authorizes(a, "publish_report", "v7", NOW) is True
    assert approval_authorizes(a, "publish_report", "v8", NOW) is False       # v7 approval != v8
    assert approval_authorizes(a, "delete_report", "v7", NOW) is False        # wrong action
    assert approval_authorizes(_approval(decision="REJECTED"), "publish_report", "v7", NOW) is False
    assert approval_authorizes(_approval(expires_at=NOW - 1), "publish_report", "v7", NOW) is False  # expired
