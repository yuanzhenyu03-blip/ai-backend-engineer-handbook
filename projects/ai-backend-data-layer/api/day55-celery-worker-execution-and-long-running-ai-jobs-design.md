...
Executed: `python3 -m pytest -q test_day55_celery_worker_execution.py` -> **40 passed**
(Python 3.10.12, pydantic 2.5.0, pytest 7.4.3). Full `projects/ai-backend-data-layer/api/` suite -> **388 passed**.
...
**Lease expiry is not re-authorization (F1 + P1).** A lease is temporary ownership; its expiry says nothing about the
external Provider call. A redelivery OR a lease-expiry re-claim — even by a new Worker — returns `RECONCILE_ONLY` and
transitions the Job to `PENDING_RECONCILIATION` whenever the open `Attempt` carries EITHER kind of external-execution
evidence:

```text
provider_request_id           = STRONG evidence: the Provider request was opened.
provider_dispatch_started_at  = CONSERVATIVE marker: the external call MAY have started.
```

The conservative marker closes a real recovery gap (P1): a Worker can dispatch to the Provider and then OOM/be killed
BEFORE it records `provider_request_id`, so a MISSING request id does NOT prove the Provider did not execute (the call
may already have run and incurred cost). The marker is persisted BEFORE the call leaves the process, and from that
commit the system never assumes "Provider definitely not executed." Only a first claim on an Attempt with NEITHER piece
of evidence is granted execution authority. A false positive (marker set, request not actually sent yet when the Worker
crashed) is the ACCEPTED safety-first trade-off — we never trade a possible duplicate paid Provider call + duplicate
cost for an automatic retry. A provider idempotency key reduces the blast radius of a retry but is NOT a reason to treat
unknown external execution as a safe retry.
...
Persist a guarded claim, Attempt, and correlation evidence BEFORE the Provider call. The strict order is:

```text
guarded claim
-> durable external-call-may-have-started marker (provider_dispatch_started_at)  [P1]
-> Provider request OUTSIDE any DB transaction
-> persist provider_request_id as soon as it is available
-> validate / guarded terminal transition
```

`mark_provider_dispatch_starting` persists the conservative marker BEFORE the call (P1); `record_provider_request_id`
records the strong evidence as soon as it is available, attributed to the REAL parent Job via the durable
`attempt_id -> job_id` path with `attempt_id` + `provider_request_id` + `correlation_id` as repair evidence (F3). A long
Provider call stays OUTSIDE any database transaction. If
...
| P1: OOM after dispatch marker but before request id -> reconciliation-only, zero re-calls, unknown cost retained, same Attempt/key | LOCAL CONTROL-FLOW | `test_p1_oom_after_dispatch_marker_before_request_id_reconciles_no_recall`, `test_p1_claim_reconciles_on_marker_only_even_without_request_id` |
| P1: accepted safety-first false positive (marker set, request maybe not sent) still reconciles, never retries; marker persisted before the call | LOCAL CONTROL-FLOW | `test_p1_marker_persisted_but_request_never_sent_still_reconciles_accepted_false_positive`, `test_p1_dispatch_marker_persisted_before_provider_call_in_worker_flow` |
...
`provider_request_id` / `provider_dispatch_started_at` (the conservative external-call marker) / `(schema_name, schema_version)` fields are new facts MODELED in-memory. A real deployment adds
